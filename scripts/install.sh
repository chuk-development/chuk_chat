#!/usr/bin/env bash
# CoWork host installer (§5: "install = one shell script + connect").
#
...
#   3. dirs       — create $COWORK_HOME and its subdirectories.
...
UNIT_TEMPLATE="${SCRIPT_DIR}/cowork-manager.service"
...
IMAGE_TAG="${COWORK_SANDBOX_IMAGE:-cowork-base:latest}"
PULL_IMAGE=""
RUNTIME="${COWORK_RUNTIME:-}"
SANDBOX_KIND="${COWORK_SANDBOX_KIND:-docker}"
DRY_RUN=0
DO_RUNTIME=1
DO_IMAGE=1
DO_ENV=1
...
Usage: install.sh [options]
  ...
  --prefix DIR        install root for state and the launcher (default ~/.cowork)
  --tag REF           image tag to build (default cowork-base:latest)
...
while [ $# -gt 0 ]; do
    case "$1" in
        --prefix) COWORK_HOME="${2:?--prefix needs a directory}"; shift 2 ;;
        --prefix=*) COWORK_HOME="${1#*=}"; shift ;;
...
check_writable_parent "${COWORK_HOME}" "install prefix"
if [ "${DO_SERVICE}" -eq 1 ]; then
...
# 3. Directories
# ---------------------------------------------------------------------------
step 3/5 "directories under ${COWORK_HOME}"
for dir in "${COWORK_HOME}" "${COWORK_HOME}/agents" "${COWORK_HOME}/logs" "${BIN_DIR}"; do
    if [ -d "${dir}" ]; then
        say "exists: ${dir}"
    else
        say "create: ${dir}"
...
# Generated by cowork install.sh — edit the checkout, not this file.
set -euo pipefail
exec \"${VENV_BIN}\" \"\$@\""
