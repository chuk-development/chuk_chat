...
void main() {
  ...
  setUp(() async {
    ...
    CoworkRelayLink.instance.sessionKey.value = session;
    loader.attach();
  });
  ...
  void turn(String q, int qMid, String a, int aMid) {
    controller.emit(CoworkRelayUser(q, replay: true, mid: qMid));
    controller.emit(CoworkRelayDelta(a, replay: true, mid: aMid));
  }

  CoworkRelayDone pageEnd({required bool hasMore, required int oldestMid, int? beforeId}) =>
      CoworkRelayDone(
        reason: 'replay',
        finalAnswer: null,
        iterations: 0,
