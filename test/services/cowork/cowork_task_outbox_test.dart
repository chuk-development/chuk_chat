...
void main() {
  ...
  tearDown(() {
    ...
    CoworkTaskOutbox.now = () => DateTime.now().toUtc();
  });

  test('a queued prompt survives a restart, verbatim and in order', () async {
    await CoworkTaskOutbox.enqueue(sessionKey: 't', prompt: 'erste');
    ...
    final tasks = await CoworkTaskOutbox.pendingFor('t');
  ...
  test('threads keep their own queues', () async {
    await CoworkTaskOutbox.enqueue(sessionKey: 'a', prompt: 'x');
    await CoworkTaskOutbox.enqueue(sessionKey: 'b', prompt: 'y');
    expect((await CoworkTaskOutbox.pendingFor('a')).single.prompt, 'x');
    expect((await CoworkTaskOutbox.pendingFor('b')).single.prompt, 'y');
  });

  test('a flush sends oldest first and empties the queue', () async {
    await CoworkTaskOutbox.enqueue(sessionKey: 't', prompt: 'eins');
    await CoworkTaskOutbox.enqueue(sessionKey: 't', prompt: 'zwei');
    final sent = <String>[];
    final count = await CoworkTaskOutbox.flush('t', (task) async {
      sent.add(task.prompt);
    });
    expect(count, 2);
    expect(sent, ['eins', 'zwei']);
    expect(await CoworkTaskOutbox.pendingFor('t'), isEmpty);
    expect(disk, isEmpty);
  });
  ...
  test('a flush that fails stops and keeps everything', () async {
    ...
    expect(count, 0);
    final pending = await CoworkTaskOutbox.pendingFor('t');
  ...
  test('a prompt the host never takes is given up on, not looped', () async {
    await CoworkTaskOutbox.enqueue(sessionKey: 't', prompt: 'giftig');
    for (int i = 0; i < CoworkTaskOutbox.maxAttempts; i++) {
      await CoworkTaskOutbox.flush('t', (task) async => throw 'no');
  ...
  test('a send with the device offline lands in the outbox with a real '
    ...
    final pending = await CoworkTaskOutbox.pendingFor('thread-1');
  ...
  test('a prompt typed offline goes out on the next flush, once', () async {
    ...
    final sent = <String>[];
    expect(
      await CoworkTaskOutbox.flush('thread-1', (task) async {
        sent.add(task.prompt);
      }),
      1,
    );
    ...
    expect(await CoworkTaskOutbox.flush('thread-1', (_) async {}), 0);
  ...
  test('a flush that fails backs off instead of hammering', () async {
    await CoworkTaskOutbox.enqueue(sessionKey: 't', prompt: 'eins');
    expect(await CoworkTaskOutbox.flush('t', (_) async => throw 'no host'), 0);
    final afterFirst = (await CoworkTaskOutbox.pendingFor('t')).single;
    ...
    int tries = 0;
    for (int i = 0; i < 3; i++) {
      await CoworkTaskOutbox.flush('t', (_) async {
        tries++;
      });
    }
    expect(tries, 0);
    expect((await CoworkTaskOutbox.pendingFor('t')).single.attempts, 1);

    // Once the wait is over it is tried again.
    clock = clock.add(const Duration(seconds: 2));
    final sent = <String>[];
    expect(
      await CoworkTaskOutbox.flush('t', (task) async => sent.add(task.prompt)),
      1,
    );
    expect(sent, ['eins']);
  });
  ...
  test('the wait grows with every failure, up to the cap', () async {
    ...
    for (int i = 0; i < 3; i++) {
      await CoworkTaskOutbox.flush('t', (_) async => throw 'no');
      final task = (await CoworkTaskOutbox.pendingFor('t')).single;
