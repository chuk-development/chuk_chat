...
import 'cowork_relay_client_test.dart' show FakeExecutorHost, FakeRelaySocket;
...
/// [CoworkRelaySecretRequest]; `sendSecrets` seals the whole set with the
...
void main() {
  ...
  test('a secret_request frame surfaces with names, purpose and thread',
    ...
    final request = events.whereType<CoworkRelaySecretRequest>().single;
  ...
  test('fromPayload drops a request with no id', () {
    expect(
      CoworkRelaySecretRequest.fromPayload(<String, dynamic>{
        'type': 'secret_request',
        'names': ['A'],
      }),
      isNull,
