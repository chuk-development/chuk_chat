...
void main() {
  ...
  setUp(() {
    ...
    CoworkQueuedMarks.readRows = (key) => store[key];
    CoworkQueuedMarks.writeRows = (key, rows) async => store[key] = rows;
  });
  ...
  test('the mark comes off when the prompt goes out', () async {
    ...
    expect(
      await CoworkQueuedMarks.clearMark(sessionKey: 't', queueId: 'q1'),
      isTrue,
    );
  ...
  test('the marks a transcript holds are what a rewrite must carry over', () {
    final marks = CoworkQueuedMarks.marksIn(<ChatMessage>[
      ChatMessage(
        role: 'user',
        text: 'wartet',
