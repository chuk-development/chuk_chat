...
class FakeReconnectHost {
  ...
  late final AgentsReconnect _initiator;
  ...
  Future<void> start() async {
    _initiator = AgentsReconnect.initiator(
...
void main() {
  test('reconnect: no code, mutual auth, then sealed tasks + results flow',
    ...
    final appKeyPair = await AgentsDeviceKeys.generate();
    ...
    final stored = AgentsStoredPairing(
    ...
    expect(events.whereType<AgentsRelayDelta>().single.text, 'hi');
  ...
  test('reconnect against an imposter host (wrong key) fails, no channel',
    ...
    final imposterKeyPair = await AgentsDeviceKeys.generate();
    final appKeyPair = await AgentsDeviceKeys.generate();
    final appPub = await appKeyPair.extractPublicKey();
