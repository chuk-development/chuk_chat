...
///
/// The vector fixture `test/fixtures/agents_pairing_vectors.json` is generated
/// by `common/agents_crypto/tests/gen_pairing_vectors.py`. X25519 and Ed25519
...
void main() {
  ...
  final Map<String, dynamic> vectors =
      jsonDecode(
            File(
              'test/fixtures/agents_pairing_vectors.json',
            ).readAsStringSync(),
          )
  ...
  Future<SimpleKeyPair> x25519FromSeed(String seedB64) =>
      AgentsPairingCrypto.x25519.newKeyPairFromSeed(base64Decode(seedB64));

  Future<SimpleKeyPair> edFromSeed(String seedB64) =>
      AgentsDeviceKeys.fromSeed(base64Decode(seedB64));

  group('Agents pairing cross-language vector', () {
    ...
    test('commitment over A matches the fixture', () async {
      final commitment = AgentsPairingCrypto.commitment(
    ...
    test('full handshake reproduces SAS, MACs and channel key byte-for-byte',
      ...
      expect(initiator.state, AgentsPairingState.completed);
  ...
  group('Agents pairing state machine', () {
    Future<List<AgentsPairing>> freshPair({
      String initiatorDigits = '428913',
      String? joinerCode,
      AgentsApprovedDevices? initiatorStore,
      AgentsApprovedDevices? joinerStore,
    }) async {
      final initiator = await AgentsPairing.initiator(
        deviceId: 'client-01',
    ...
    Future<void> runToConfirmed(
      AgentsPairing initiator,
      AgentsPairing joiner,
    ) async {
      final commit = initiator.createCommit();
      joiner.onCommit(commit);
      final reveal = await initiator.onPubkey(joiner.createPubkey());
    ...
    test('happy path: same key + SAS, mutual device trust', () async {
      final initStore = AgentsApprovedDevices.empty();
      final joinStore = AgentsApprovedDevices.empty();
    ...
    test('channel key is hidden before confirmation', () async {
      ...
      expect(
        ...
        throwsA(
          isA<AgentsPairingException>().having(
            (e) => e.rejection,
            'rejection',
    ...
    test(
      ...
      final atkLegToInitiator = await AgentsPairingCrypto.x25519.newKeyPair();
      final atkLegToJoiner = await AgentsPairingCrypto.x25519.newKeyPair();
      ...
      joiner.onCommit(<String, dynamic>{
        ...
        'commitment': base64.encode(AgentsPairingCrypto.commitment(atkPubToJoiner)),
      ...
      final k1Secret = await AgentsPairingCrypto.x25519.sharedSecretKey(
      ...
      final t1 = AgentsPairingCrypto.transcript(realA, atkPubToInitiator);

      // 4. The attacker can only guess PC. A wrong guess is rejected.
      final guessed = await AgentsPairingCrypto.deriveConfirmMac(
        k1,
      ...
      await expectLater(
        ...
        throwsA(
          isA<AgentsPairingException>().having(
            (e) => e.rejection,
            'rejection',
            AgentsPairingRejection.macMismatch,
          ),
        ),
      );
      expect(initiator.state, AgentsPairingState.aborted);
      expect(() => initiator.channelKey, throwsA(isA<AgentsPairingException>()));
      ...
      final withRealPc = await AgentsPairingCrypto.deriveConfirmMac(
        k1,
        AgentsPairingCrypto.confirmDLabel,
        t1,
    ...
    test('MITM swapping A is caught by the commitment', () async {
      ...
      final attacker = await AgentsPairingCrypto.x25519.newKeyPair();
      ...
      await expectLater(
        ...
        throwsA(
          isA<AgentsPairingException>().having(
            (e) => e.rejection,
            'rejection',
            AgentsPairingRejection.commitmentMismatch,
          ),
        ),
      );
      expect(joiner.state, AgentsPairingState.aborted);
    });
      ...
      expect(
        ...
        throwsA(
          isA<AgentsPairingException>().having(
            (e) => e.rejection,
            'rejection',
            AgentsPairingRejection.expired,
          ),
        ),
      );
    });
    ...
    test('channel id mismatch is rejected', () async {
      ...
      final commit = initiator.createCommit();
      final wrongJoiner = await AgentsPairing.joiner(
        deviceId: 'desktop-01',
        deviceKeyPair: await AgentsDeviceKeys.generate(),
      ...
      expect(
        ...
        throwsA(
          isA<AgentsPairingException>().having(
            (e) => e.rejection,
            'rejection',
            AgentsPairingRejection.channelMismatch,
          ),
        ),
      );
    });
