...
void main() {
  test('loadOrCreateIdentity persists a STABLE device id + key across calls',
    ...
    final store = AgentsPairingStore(backend: backend);
    ...
    final firstSeed = await AgentsDeviceKeys.exportPrivateKeySeedBase64(first.keyPair);
    final secondSeed =
        await AgentsDeviceKeys.exportPrivateKeySeedBase64(second.keyPair);
    ...
    final reopened =
        await AgentsPairingStore(backend: backend).loadOrCreateIdentity();
  ...
  test('savePairing / loadPairing round-trips every field', () async {
    ...
    final pairing = AgentsStoredPairing(
  ...
  test('clearPairing forgets the trust but keeps the device identity', () async {
    ...
    final hostPub = await (await AgentsDeviceKeys.generate()).extractPublicKey();
  ...
  test('the default backend is flutter_secure_storage, never SharedPreferences',
    ...
    expect(AgentsPairingStore().backend, isA<FlutterSecureKeyValueStore>());
  });
  ...
  test('the device identity survives a restart and still authenticates a '
    ...
    final run1 = AgentsPairingStore(backend: backend);
    final identity1 = await run1.loadOrCreateIdentity();
    // What the host approves and persists on its side.
    final hostKeyPair = await AgentsDeviceKeys.generate();
    ...
    final run2 = AgentsPairingStore(backend: backend);
    ...
    final host = AgentsReconnect.initiator(
    ...
    final app = AgentsReconnect.joiner(
    ...
    final reinstalled = AgentsPairingStore(backend: _MemoryStore());
    final otherIdentity = await reinstalled.loadOrCreateIdentity();
    final host2 = AgentsReconnect.initiator(
      deviceId: hostDeviceId,
      deviceKeyPair: hostKeyPair,
      peerDeviceId: appDeviceId,
      peerPublicKey: appPubAtPairing,
  ...
  test('a corrupt or future-version record parses as no pairing', () async {
    expect(AgentsStoredPairing.tryParse('not json'), isNull);
    expect(AgentsStoredPairing.tryParse('{"version":2}'), isNull);
    expect(
      AgentsStoredPairing.tryParse(
