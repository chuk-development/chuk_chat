...
import 'package:cowork/utils/io_helper.dart' as ioh;
...
void main() {
  ...
  setUp(() async {
    ...
    CoworkChatCacheMigration.reset();
    tmp = await io.Directory.systemTemp.createTemp('cowork-migration-');
    CoworkChatCacheMigration.chatDirProvider = () async =>
  ...
  test("yesterday's P2b file becomes the thread, with its own dates", () async {
    ...
    CoworkChatCacheMigration.hasLocalCopy = (_, _) async => false;

    final n = await CoworkChatCacheMigration.migrateJsonCache('user-7');
    expect(n, 1);
  ...
  test('a thread SQLite already holds keeps the newer row; the file is '
    ...
    var writes = 0;
    CoworkChatCacheMigration.hasLocalCopy = (_, _) async => true;
    CoworkChatCacheMigration.writer =
  ...
  test('no cache directory: nothing to do, no error', () async {
    CoworkChatCacheMigration.chatDirProvider = () async => null;
    expect(await CoworkChatCacheMigration.migrateJsonCache('user-7'), 0);
  });
  ...
  test(
    ...
    () async {
      ...
      CoworkChatCacheMigration.hasLocalCopy = (_, key) async =>
          key == 'agent-kept';
      final dropped = await CoworkChatCacheMigration.dropOrphanCursors(
        'user-7',
      );
      expect(dropped, 1);
  ...
  test(
    ...
    () async {
      await CoworkChatStore.replaceThread('agent-mem', <Map<String, dynamic>>[
