...
void main() {
  ...
  testWidgets('a system back turns into onBack (PopScope)', (tester) async {
    ...
    expect(back, 1);
    expect(find.byType(MobileChatScreen), findsOneWidget, reason: 'still there');
  });

  testWidgets('a swipe from the left edge goes back; a swipe from the middle does not',
      (tester) async {
    int back = 0;
    await pumpPhone(
      tester,
    ...
    await tester.flingFrom(const Offset(195, 400), const Offset(240, 0), 1500);
