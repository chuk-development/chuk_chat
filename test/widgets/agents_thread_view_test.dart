...
void main() {
  ...
  testWidgets('chat mounts before controller initialization finishes', (
    ...
    final ready = Completer<AgentsRelayController>();
  ...
  group('persistent pairing', () {
    Future<AgentsPairingStore> seededStore() async {
      final store = AgentsPairingStore(backend: _MemoryStore());
      final hostPub = await (await AgentsDeviceKeys.generate())
          .extractPublicKey();
    ...
    Future<(List<FakeRelayController>, AgentsPairingStore)> pumpPersistent(
      WidgetTester tester, {
      bool hostAway = false,
    }) async {
    ...
    testWidgets('resume replaces a suspended paired socket immediately', (
      ...
      final state = tester.state<AgentsThreadViewState>(
        find.byType(AgentsThreadView),
      );
          ...
          _app(
            AgentsThreadView(
              controllerBuilder: () => ++builds == 1
                  ? Future<AgentsRelayController>.value(first)
                  : gate.future,
              sessionSource: const _FakeSessionSource(),
              pairingStore: store,
            ),
    ...
    testWidgets('a dropped connection does NOT end a run on the host', (
      ...
      AgentsRunLedger.instance.begin('default');

      controllers.single.set(
        const AgentsRelayState(phase: AgentsRelayPhase.closed),
      );
      await tester.pump();
  ...
  group('the imported chat, live', () {
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pumpAndSettle();
        controller.emit(
          const AgentsRelayRunState(sessionKey: 'host-typing', state: 'idle'),
        );
        await tester.pumpAndSettle();
        expect(find.byKey(const ValueKey('host-run-typing')), findsNothing);
        expect(AgentsRunLedger.instance.isRunning('host-typing'), isFalse);
        controller.emit(
          const AgentsRelayRunState(
            sessionKey: 'host-typing',
            state: 'running',
    ...
    testWidgets('phone reaction survives transcript remount and toggles off', (
      ...
      await tester.pumpAndSettle();
      controller.emit(
        const AgentsRelayRunState(sessionKey: 'reaction-chat', state: 'idle'),
      );
      controller.emit(
        const AgentsRelayUser('A message worth reacting to', mid: 1),
      );
    ...
    group('the outbox', () {
      ...
      setUp(() {
        ...
        AgentsQueuedMarks.readRows = (_) => null;
        AgentsQueuedMarks.writeRows = (_, _) async {};
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(AgentsRelayLink.instance.sessionKey.value, 'amber-otter-2');
        final screen = tester.widget<ChukChatUIDesktop>(
          find.byType(ChukChatUIDesktop),
        );
    ...
    testWidgets('a replay that lands mid-run does not tear the screen down', (
      ...
      controller.emit(const AgentsRelayUser('older turn', mid: 1));
      controller.emit(const AgentsRelayDone(reason: 'replay', replay: true));
      await tester.pumpAndSettle();
      ...
      AgentsRunLedger.instance.finish('thread-1', reason: 'finished');
      await tester.pumpAndSettle();
    ...
    testWidgets('a live done is acknowledged to the host, exactly once', (
      ...
      controller.emit(
        const AgentsRelayDone(reason: 'finished', runId: 'run-9'),
      );
      controller.emit(
        const AgentsRelayDone(reason: 'finished', runId: 'run-9'),
      );
