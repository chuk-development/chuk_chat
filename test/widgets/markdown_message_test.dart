...
''', width: 260);
  ...
  // ── Upstream guards: dollar amounts must not become math ──────────────
  //
  // From chuk_chat master's own markdown_message_test.dart. They are kept
  // verbatim, with their own harness, because they assert against the plain
  // renderer (no SelectionArea, default theme) the upstream fix was written
  // for.

  Widget buildUpstreamMarkdown(String text) {
    return MaterialApp(
      home: Scaffold(
  ...
  testWidgets('still renders explicit escaped inline math', (tester) async {
    await tester.pumpWidget(buildUpstreamMarkdown(r'The identity is \(x + y\).'));
