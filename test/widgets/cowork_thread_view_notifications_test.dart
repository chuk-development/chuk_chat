...
void main() {
  ...
  setUp(() async {
    ...
    CoworkNotifications.instance.threadLabel = (String key) => 'Chief of Staff';
  });
  ...
  testWidgets('a live done while backgrounded shows one generic toast',
    ...
    controller.emit(const CoworkRelayDone(reason: 'finished', runId: 'run-1'));
    await tester.pumpAndSettle();
  ...
  testWidgets('a live done in the foreground shows nothing', (tester) async {
    ...
    controller.emit(const CoworkRelayDone(reason: 'finished', runId: 'run-2'));
    await tester.pumpAndSettle();
