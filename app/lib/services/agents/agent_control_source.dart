...
///
/// Status comes back as [AgentsRelayAgentStatus] — as the answer to a request,
...
class RelayAgentControlSource implements AgentControlSource {
  RelayAgentControlSource({SkillsSource? skills})
    ...
    AgentsRelayLink.instance.controller.addListener(_onController);
    _onController();
    _skills.addListener(_onSkills);
  }

  final SkillsSource _skills;
  StreamSubscription<AgentsRelayAgentStatus>? _sub;
  ...
  Future<void> refresh(String sessionKey) async {
    ...
    if (controller is! AgentsAgentStatusControl) {
  ...
  void dispose() {
    _disposed = true;
    AgentsRelayLink.instance.controller.removeListener(_onController);
    _sub?.cancel();
    _sub = null;
  ...
  void _onStatus(AgentsRelayAgentStatus event) {
    if (_disposed) return;
...
/// the constructor call lives in `agents_shell_state.dart`, which another
