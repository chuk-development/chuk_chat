...
class AgentsReplayGuard {
  ...
  void commit(AgentsFrame frame) {
    _checkTimestamp(frame);
    _checkSeq(frame);
    _lastSeq = frame.seq;
  }

  void _checkTimestamp(AgentsFrame frame) {
    ...
    if (skew.abs() > window.inMilliseconds) {
      throw const AgentsFrameRejectedException(
        AgentsFrameRejection.timestampOutOfWindow,
      );
    }
  }

  void _checkSeq(AgentsFrame frame) {
    final last = _lastSeq;
    if (last != null && frame.seq <= last) {
      throw const AgentsFrameRejectedException(
        AgentsFrameRejection.replayedSequence,
      );
    }
  }
}
