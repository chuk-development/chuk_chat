...
void main() {
  ...
  testWidgets('on a phone the screen opens with the camera', (tester) async {
    ...
    expect(
      find.byKey(const ValueKey<String>('agents-pairing-use-code')),
      findsOneWidget,
    );
        ...
        home: Scaffold(
          body: Builder(
            builder: (context) => TextButton(
              onPressed: () async {
                got = await Navigator.of(context).push<AgentsPairingInvite>(
                  MaterialPageRoute<AgentsPairingInvite>(
                    builder: (_) => AgentsPairingPage(
  ...
  testWidgets('a stray QR code is ignored, the camera stays up', (
    ...
    expect(find.byKey(const ValueKey<String>('fake-camera')), findsOneWidget);
    expect(
      find.byKey(const ValueKey<String>('agents-pairing-message')),
      findsOneWidget,
    );
  ...
  testWidgets('a refused camera falls through to the code field', (
    ...
    expect(
      find.byKey(const ValueKey<String>('agents-pairing-code-field')),
      findsOneWidget,
    );
    expect(
  ...
  testWidgets('"enter the code instead" is the same result as a scan', (
    ...
    await tester.tap(
      find.byKey(const ValueKey<String>('agents-pairing-connect')),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('on a desktop the code field is what opens', (tester) async {
    ...
    final camera = tester.widget<TextButton>(
      find.byKey(const ValueKey<String>('agents-pairing-use-camera')),
    );
