...
void main() {
  test('a turn the app painted itself is not appended twice', () {
    ...
    expect(texts(CoworkReplayLoader.appendWithoutRepeats(existing, delta)), [
      'user:erste frage',
      'ai:erste antwort',
      'user:Nimm das bitte im Browser',
      'ai:mache ich',
  ...
  test('a short delta never shrinks the thread', () {
    ...
    final merged = CoworkReplayLoader.appendWithoutRepeats(existing, delta);
  ...
  test('an empty side is left alone', () {
    expect(
      texts(CoworkReplayLoader.appendWithoutRepeats(<Map<String, String>>[], [
        user('a'),
      ])),
      ['user:a'],
    );
    expect(
      texts(
        CoworkReplayLoader.appendWithoutRepeats([
          user('a'),
        ], <Map<String, String>>[]),
      ),
      ['user:a'],
...
void _pagingTests() {
  test('an older page does not repeat what the cache already shows', () {
    ...
    expect(texts(CoworkReplayLoader.appendWithoutRepeats(page, cached)), [
      'user:alt',
      'ai:ALT',
      'user:a',
      'ai:A',
...
void _repairTests() {
  test('the first load after the update keeps no cursor', () async {
    ...
    final loader = CoworkReplayLoader.instance..reset();
    await loader.load();
