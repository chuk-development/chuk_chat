...
void main() {
  ...
  test('a received agent_list overwrites the persisted roster', () async {
    ...
    second.applyHostNames(<CoworkHostAgentName>[
      CoworkHostAgentName(agentId: agent.id, name: 'host-name'),
      const CoworkHostAgentName(agentId: 'agent-2', name: 'cobalt-tern'),
    ], peerDeviceId: null);
    await nextLaunch().flush();
  ...
  test('a deleted coworker does not come back', () async {
    ...
    second.applyHostNames(<CoworkHostAgentName>[
      CoworkHostAgentName(agentId: agent.id, name: 'sable-wren'),
  ...
  test('a load never undoes what the pairing already put in the roster', () async {
    ...
    expect(
      second.agents.where((CoworkAgent a) => a.id == host.id),
      hasLength(1),
    );
  });
}
