...
void main() {
  ...
  final loader = CoworkReplayLoader.instance;
  ...
  test('a scripted replay becomes user and assistant rows', () async {
    await replay(const <CoworkRelayInbound>[
      CoworkRelayRunState(sessionKey: sessionKey, state: 'idle'),
      CoworkRelayUser('do the thing', mid: 1),
      CoworkRelayDelta('all ', replay: true, mid: 2),
      CoworkRelayDelta('set', replay: true, mid: 3),
  ...
  test('a replayed tool lands as a completed tool call on the answer',
    ...
    await replay(const <CoworkRelayInbound>[
      CoworkRelayUser('do the thing', mid: 1),
      CoworkRelayDelta('working', replay: true, mid: 2),
      CoworkRelayTool(
        'run_command',
        arguments: 'ls /nope',
        result: 'No such file',
  ...
  test('a replayed file becomes a sandbox artifact block', () async {
    await replay(<CoworkRelayInbound>[
      const CoworkRelayUser('make me a csv', mid: 1),
      const CoworkRelayDelta('here you go', replay: true, mid: 2),
      CoworkRelayFile(
        name: 'report.csv',
        mimeType: 'text/csv',
        declaredSize: 3,
  ...
  test('a LIVE file or child agent is left to the ledger', () async {
    await replay(<CoworkRelayInbound>[
      const CoworkRelayUser('go', mid: 1),
      const CoworkRelayDelta('done', replay: true, mid: 2),
      CoworkRelayFile(
        name: 'live.csv',
        mimeType: 'text/csv',
        declaredSize: 1,
      ...
      const CoworkRelaySubagent(subagentId: 'sa_live', title: 't', state: 'running'),
      const CoworkRelayDone(reason: 'replay', replay: true),
    ]);
  ...
  test('replayed child-agent states fold into one card, last state wins',
      () async {
    await replay(<CoworkRelayInbound>[
      const CoworkRelayUser('research it', mid: 1),
      const CoworkRelayDelta('on it', replay: true, mid: 2),
  ...
  test('a replayed, decided approval is a card with nothing to press',
      () async {
    await replay(<CoworkRelayInbound>[
      const CoworkRelayUser('publish it', mid: 1),
      ...
      const CoworkRelayDelta('published', replay: true, mid: 3),
      const CoworkRelayDone(reason: 'replay', replay: true),
    ]);
  ...
  test('a replayed open approval on a run still in flight is drawn as live',
      () async {
    await replay(<CoworkRelayInbound>[
      const CoworkRelayRunState(sessionKey: sessionKey, state: 'running', runId: 'r1'),
      const CoworkRelayUser('publish it', mid: 1),
      const CoworkRelayApprovalRequest(
        approvalId: 'ap1',
        action: 'herenow_publish',
  ...
  test('a replayed open approval on an idle run is history, not a prompt',
      () async {
    await replay(<CoworkRelayInbound>[
      const CoworkRelayRunState(sessionKey: sessionKey, state: 'idle'),
      const CoworkRelayUser('publish it', mid: 1),
      const CoworkRelayApprovalRequest(
        approvalId: 'ap1',
        action: 'herenow_publish',
  ...
  test('a second full replay overwrites, it never duplicates', () async {
    const script = <CoworkRelayInbound>[
      CoworkRelayUser('do the thing', mid: 1),
      CoworkRelayDelta('all set', replay: true, mid: 2),
      CoworkRelayDone(reason: 'replay', replay: true),
    ];
    await replay(script);
  ...
  test('the cursor advances and is persisted per session', () async {
    ...
    await replay(const <CoworkRelayInbound>[
      CoworkRelayUser('do the thing', mid: 4),
      CoworkRelayDelta('all set', replay: true, mid: 9),
      CoworkRelayDone(reason: 'replay', replay: true),
    ]);
  ...
  test('forgetting the cursor makes the next replay replace instead of append',
    ...
    await replay(const <CoworkRelayInbound>[
      CoworkRelayUser('why', mid: 1),
      CoworkRelayDelta('because', replay: true, mid: 2),
      CoworkRelayDone(reason: 'replay', replay: true),
    ]);
  ...
  test('a replayed thought lands in the row it belongs to', () async {
    ...
    await replay(const <CoworkRelayInbound>[
      CoworkRelayUser('why', mid: 1),
      CoworkRelayReasoning('weighing it up', replay: true, mid: 2),
      CoworkRelayDelta('because', replay: true, mid: 3),
      CoworkRelayDone(reason: 'replay', replay: true),
    ]);
  ...
  test('a LIVE thought is left to the adapter', () async {
    await replay(const <CoworkRelayInbound>[
      CoworkRelayUser('why', mid: 1),
      CoworkRelayReasoning('live thinking'),
      CoworkRelayDelta('because', replay: true, mid: 2),
      CoworkRelayDone(reason: 'replay', replay: true),
    ]);
  ...
  test('a delta replay above the cursor appends to what is cached', () async {
    await replay(const <CoworkRelayInbound>[
      CoworkRelayUser('first', mid: 1),
      CoworkRelayDelta('first answer', replay: true, mid: 2),
      CoworkRelayDone(reason: 'replay', replay: true),
    ]);
    expect(rowsFor(sessionKey), hasLength(2));
    ...
    await replay(
      const <CoworkRelayInbound>[
        CoworkRelayUser('second', mid: 3),
        CoworkRelayDelta('second answer', replay: true, mid: 4),
        CoworkRelayDone(reason: 'replay', replay: true),
      ],
      afterId: 2,
    );
  ...
  test('an empty delta replay leaves the cache alone', () async {
    ...
    await replay(
      const <CoworkRelayInbound>[CoworkRelayDone(reason: 'replay', replay: true)],
      afterId: 2,
    );
  ...
  test('a replayed run terminal closes the answer and marks it ready',
    ...
    await replay(const <CoworkRelayInbound>[
      CoworkRelayUser('do the thing', mid: 1),
      CoworkRelayDelta('the answer', replay: true, mid: 2),
      CoworkRelayDone(
        reason: 'finished',
        replay: true,
        whileAway: true,
      ...
      CoworkRelayUser('and again', mid: 3),
      CoworkRelayDelta('the second answer', replay: true, mid: 4),
      CoworkRelayDone(reason: 'replay', replay: true),
    ]);
  ...
  test('live frames are ignored: the loader only owns history', () async {
    await replay(const <CoworkRelayInbound>[
      CoworkRelayDelta('a live token'),
      CoworkRelayTool('run_command', exitCode: 0),
      CoworkRelayReasoning('live thinking'),
      CoworkRelayDone(reason: 'finished', iterations: 1),
      CoworkRelayDone(reason: 'replay', replay: true),
    ]);
  ...
  test('the run_state header routes a replay to the session it names',
    ...
    controller.emit(
      const CoworkRelayRunState(sessionKey: 'other-thread', state: 'idle'),
    );
    controller.emit(const CoworkRelayUser('over there', mid: 1));
  ...
  test('two overlapping replay requests land in their own threads (F1)',
    ...
    controller.emit(
      const CoworkRelayRunState(sessionKey: 'default', state: 'idle'),
    );
    controller.emit(const CoworkRelayUser('first question', mid: 1));
    await _drain();
    // Second request while the first answer is still streaming.
    loader.expect('host:peer', afterId: 10);
    controller.emit(const CoworkRelayDelta('first answer', replay: true, mid: 2));
    controller.emit(const CoworkRelayDone(reason: 'replay', replay: true));
    // Now the host answers the second request.
    controller.emit(
      const CoworkRelayRunState(sessionKey: 'host:peer', state: 'idle'),
    );
    controller.emit(const CoworkRelayUser('other question', mid: 11));
    controller.emit(const CoworkRelayDelta('other answer', replay: true, mid: 12));
    controller.emit(const CoworkRelayDone(reason: 'replay', replay: true));
    await _drain();
  ...
  test('a second request for the same thread queues behind the answer in '
    ...
    controller.emit(const CoworkRelayUser('q1', mid: 1));
    await _drain();
    // Asked again mid-stream: the first answer must still commit whole.
    loader.expect(sessionKey);
    controller.emit(const CoworkRelayDelta('a1', replay: true, mid: 2));
    controller.emit(const CoworkRelayDone(reason: 'replay', replay: true));
    await _drain();
    ...
    controller.emit(const CoworkRelayUser('q1', mid: 1));
    controller.emit(const CoworkRelayDelta('a1', replay: true, mid: 2));
    controller.emit(const CoworkRelayUser('q2', mid: 3));
    controller.emit(const CoworkRelayDelta('a2', replay: true, mid: 4));
    controller.emit(const CoworkRelayDone(reason: 'replay', replay: true));
    await _drain();
  ...
  test('a LIVE user frame is ignored like every other live frame (F13)',
      () async {
    await replay(const <CoworkRelayInbound>[
      CoworkRelayUser('not history', replay: false),
      CoworkRelayUser('history', mid: 1),
      CoworkRelayDone(reason: 'replay', replay: true),
    ]);
