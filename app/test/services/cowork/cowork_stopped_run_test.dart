...
void main() {
  ...
  final Duration ceiling = CoworkRunLedger.ceiling;
  final Duration ceilingGrace = CoworkRunLedger.ceilingGrace;
  final Duration stopGrace = CoworkRunLedger.stopGrace;
  final Duration idleHeaderGrace = CoworkRunLedger.idleHeaderGrace;
  ...
  tearDown(() async {
    CoworkRunLedger.ceiling = ceiling;
    CoworkRunLedger.ceilingGrace = ceilingGrace;
    CoworkRunLedger.stopGrace = stopGrace;
    CoworkRunLedger.idleHeaderGrace = idleHeaderGrace;
    ledger.reset();
    loader.reset();
    CoworkRelayLink.instance.reset();
    await ChatStorageService.reset();
  ...
  group('a terminal that carries no answer', () {
    test('ends the run and says it was stopped', () {
      ...
      expect(run.outcome, CoworkRunOutcome.stopped);
      expect(run.endedWithoutAnswer, isTrue);
      expect(
        coworkRunEndNotice(run.outcome!),
    ...
    test('an error terminal reads as a failure, not as a stop', () {
      ...
      final run = ledger.runFor(sessionKey)!;
      expect(run.outcome, CoworkRunOutcome.failed);
      expect(coworkRunEndNotice(run.outcome!), contains('failed'));
    });
    ...
    test('a run that answered leaves no line behind', () {
      ...
      final run = ledger.runFor(sessionKey)!;
      expect(run.outcome, CoworkRunOutcome.answered);
      expect(run.endedWithoutAnswer, isFalse);
      expect(coworkRunEndNotice(run.outcome!), isNull);
    });
    ...
    test('a streamed answer with no final_answer field still answered', () {
      ...
      expect(ledger.runFor(sessionKey)!.outcome, CoworkRunOutcome.answered);
    ...
    test('a stop with no terminal at all still ends the run', () {
      ...
      ledger.sweep(now: asked.add(CoworkRunLedger.stopGrace * 0.5));
      expect(ledger.isRunning(sessionKey), isTrue, reason: 'still in grace');

      ledger.sweep(now: asked.add(CoworkRunLedger.stopGrace * 2));

      expect(ledger.isRunning(sessionKey), isFalse);
      expect(ledger.runFor(sessionKey)!.outcome, CoworkRunOutcome.stopped);
    });
  });
  ...
  group('reconnect', () {
    test('an idle host header clears a run this client started', () {
      ...
      expect(ledger.isRunning(sessionKey), isFalse);
      expect(ledger.runFor(sessionKey)!.outcome, CoworkRunOutcome.lost);
    });
  ...
  group('the ceiling', () {
    test('a silent run is asked about, then declared lost', () {
      ...
      ledger.sweep(now: start.add(CoworkRunLedger.ceiling * 0.5));
      expect(asked, isEmpty);
      expect(ledger.isRunning(sessionKey), isTrue);

      // Past the ceiling: ask the host before accusing it.
      final probe = start.add(CoworkRunLedger.ceiling * 1.1);
      ledger.sweep(now: probe);
      expect(asked, <String>[sessionKey]);
      expect(ledger.isRunning(sessionKey), isTrue);

      // Nothing answered within the grace: the app lost the run.
      ledger.sweep(now: probe.add(CoworkRunLedger.ceilingGrace * 1.1));
      expect(ledger.isRunning(sessionKey), isFalse);
      expect(ledger.runFor(sessionKey)!.outcome, CoworkRunOutcome.lost);
      expect(
        coworkRunEndNotice(ledger.runFor(sessionKey)!.outcome!),
    ...
    test('a run that keeps working is never declared lost', () {
      ...
      var clock = start;
      for (var i = 0; i < 6; i++) {
        clock = clock.add(CoworkRunLedger.ceiling * 0.5);
    ...
    test('a host that answers the probe revives the run', () {
      final start = DateTime.now();
      ledger.begin(sessionKey);
      ledger.sweep(now: start.add(CoworkRunLedger.ceiling * 1.1));
      ...
      ledger.sweep(now: start.add(CoworkRunLedger.ceiling * 1.2));
      expect(ledger.isRunning(sessionKey), isTrue);
    });
    ...
    test('a terminal takes the run out of the sweep', () {
      ...
      ledger.sweep(now: start.add(CoworkRunLedger.ceiling * 10));
      expect(asked, 0);
      expect(ledger.runFor(sessionKey)!.outcome, CoworkRunOutcome.answered);
    });
  });
  ...
  group('the line in the thread', () {
    test('a replayed stop terminal leaves the same line', () async {
      loader.expect(sessionKey, afterId: 0);
      controller
        ..emit(const CoworkRelayRunState(sessionKey: sessionKey, state: 'idle'))
        ..emit(const CoworkRelayUser('do the thing', mid: 1))
        ..emit(
          const CoworkRelayDone(reason: 'interrupted', replay: true, runId: 'r'),
        )
        ..emit(const CoworkRelayDone(reason: 'replay', replay: true));
      await _drain();
    ...
    test('a replayed answer is never overwritten by a line', () async {
      ...
      controller
        ..emit(const CoworkRelayUser('do the thing', mid: 1))
        ..emit(const CoworkRelayDelta('half an answer', replay: true, mid: 2))
        ..emit(const CoworkRelayDone(reason: 'interrupted', replay: true))
        ..emit(const CoworkRelayDone(reason: 'replay', replay: true));
      await _drain();
