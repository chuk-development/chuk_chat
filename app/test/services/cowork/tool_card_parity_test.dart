...
final List<CoworkRelayInbound> _liveRun = <CoworkRelayInbound>[
  const CoworkRelayDelta('Let me look.'),
  CoworkRelayTool(
    'run_command',
    arguments: 'ls /tmp',
...
final List<CoworkRelayInbound> _replayedRun = <CoworkRelayInbound>[
  const CoworkRelayUser('do the thing', mid: 1),
  const CoworkRelayDelta('Let me look.', replay: true, mid: 2),
  CoworkRelayTool(
    'run_command',
    arguments: 'ls /tmp',
    argumentMap: const <String, dynamic>{'command': 'ls /tmp', 'timeout': 30},
  ...
  const CoworkRelayDelta(' Done.', replay: true, mid: 6),
  CoworkRelayDone(
    reason: 'finished',
    iterations: 3,
    runId: 'run-1',
...
void main() {
  ...
  Future<List<ToolCall>> live(List<CoworkRelayInbound> events) async {
  ...
  Future<List<ToolCall>> replayed(List<CoworkRelayInbound> events) async {
  ...
  test('the live ledger keeps the host clock for the run', () async {
    ...
    await sub.cancel();
    final run = CoworkRunLedger.instance.runFor(sessionKey)!;
