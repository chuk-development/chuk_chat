...
void main() {
  group('CoworkAutomation.fromPayload', () {
    test('reads every field the host sends', () {
      final a = CoworkAutomation.fromPayload(eventPayload())!;
    ...
    test('a watcher carries its log path and a failed one its error', () {
      final a = CoworkAutomation.fromPayload(<String, dynamic>{
        ...
        'log_path': '.cowork/automations/ab12cd34.log',
      })!;
      expect(a.isWatcher, isTrue);
      expect(a.isOver, isTrue);
      expect(a.lastError, 'exit code 3');
      expect(a.logPath, '.cowork/automations/ab12cd34.log');
      expect(a.specLabel, 'watch poll.py');
    });

    test('drops a payload without id or session', () {
      expect(CoworkAutomation.fromPayload({'type': 'automation'}), isNull);
      expect(CoworkAutomation.fromPayload({'id': 'x'}), isNull);
    });

    test('specLabel reads every spec shape', () {
      CoworkAutomation withSpec(Map<String, dynamic> spec) =>
          CoworkAutomation.fromPayload(eventPayload(spec: spec))!;
    ...
    test('toJson round-trips through fromPayload', () {
      final a = CoworkAutomation.fromPayload(eventPayload())!;
      final again = CoworkAutomation.fromPayload(a.toJson())!;
      expect(again, a);
      expect(again.nextFireAt, a.nextFireAt);
    });
  });

  group('CoworkRelayAutomation.fromPayload', () {
    test('wraps the automation with the event, run and reason', () {
      final e = CoworkRelayAutomation.fromPayload(
    ...
    test('a payload naming no automation is dropped', () {
      expect(CoworkRelayAutomation.fromPayload({'type': 'automation'}), isNull);
    });
  });

  group('CoworkRelayAutomationList.fromPayload', () {
    test('reads the entries and the scope', () {
      final list = CoworkRelayAutomationList.fromPayload(<String, dynamic>{
    ...
    test('no scope and no entries is the whole host, empty', () {
      final list = CoworkRelayAutomationList.fromPayload({'type': 'automation_list'});
  ...
  group('automationCallFromRelay', () {
    CoworkRelayAutomation relay(Map<String, dynamic> payload) =>
        CoworkRelayAutomation.fromPayload(payload)!;
