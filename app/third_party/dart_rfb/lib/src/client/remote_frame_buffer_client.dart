...
class RemoteFrameBufferClient {
  ...
  /// Tight (7) decoder state: four zlib streams that persist for the whole
  /// connection. One per client because rectangles must be decoded in order
  /// on the stream they continue.
  final TightDecoder _tightDecoder = TightDecoder();

  /// JPEG quality advertised for Tight (0..9). 6 is noVNC's default; on a
  /// 1280x800 browser page it measured ~255 KB per full frame vs 4.1 MB raw.
  static const int tightJpegQuality = 6;
      ...
      incomingMessages
        ...
        message.when(
          ...
          serverCutTextMessage:
              (final RemoteFrameBufferServerCutTextMessage _) {},
          setColorMapEntries: () {},
        );
      }),
    );
  ...
  TaskEither<Object, void> _setEncodingTypes({
    required final RawSocket socket,
      ...
      TaskEither<Object, void>.tryCatch(
        () async {
          // Order matters: the server picks the first encoding it supports.
          // Tight first so x11vnc sends compressed rectangles (zlib'd
          // copy/palette, solid fills, JPEG for photos) instead of raw pixels;
          // raw stays as the universal fallback. The quality pseudo-encoding
          // enables JPEG. No compression-level pseudo-encoding on purpose:
          // level 0 would make libvncserver emit "no-zlib" rects.
          final RemoteFrameBufferSetEncodingsMessage message =
              RemoteFrameBufferSetEncodingsMessage(
            encodingTypes: <RemoteFrameBufferEncodingType>[
              RemoteFrameBufferEncodingType.fromId(
                RemoteFrameBufferEncodingType.tightId,
              ),
              const RemoteFrameBufferEncodingType.copyRect(),
              const RemoteFrameBufferEncodingType.raw(),
              RemoteFrameBufferEncodingType.fromId(
                RemoteFrameBufferEncodingType.tightQualityId(tightJpegQuality),
              ),
            ],
          );
          logger.info('> $message');
