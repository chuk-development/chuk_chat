...
///
/// The screen produces a [AgentsPairingInvite] and nothing else; who dials it
...
class AgentsPairingPage extends StatefulWidget {
  ...
  final AgentsQrViewBuilder? qrViewBuilder;

  /// Overrides the platform check. Null asks [agentsCameraIsDefault].
  final bool? cameraAvailable;

  /// Opens the screen and hands back what the user scanned or typed, or null
  /// when they backed out.
  static Future<AgentsPairingInvite?> show(BuildContext context) {
    return Navigator.of(context).push<AgentsPairingInvite>(
      MaterialPageRoute<AgentsPairingInvite>(
        builder: (_) => const AgentsPairingPage(),
  ...
  @override
  State<AgentsPairingPage> createState() => _AgentsPairingPageState();
}

class _AgentsPairingPageState extends State<AgentsPairingPage> {
  final TextEditingController _code = TextEditingController();

  late bool _scanning = widget.cameraAvailable ?? agentsCameraIsDefault();
  ...
  void _accept(AgentsPairingInvite invite) {
    if (_handled) return;
    _handled = true;
  ...
  void _onScanned(String raw) {
    if (_handled) return;
    final invite = AgentsPairingInvite.tryParse(raw);
    if (invite == null) {
      ...
      if (mounted && _message == null) {
        setState(
          () => _message =
              'That is not a Agents code. Keep the camera '
  ...
  void _submitCode() {
    final invite = AgentsPairingInvite.tryParse(_code.text);
    if (invite == null) {
      setState(
        () => _message =
          ...
          children: <Widget>[
            ...
            if (_message != null)
              Padding(
                ...
                child: Text(
                  _message!,
                  key: const ValueKey<String>('agents-pairing-message'),
            ...
            Padding(
              ...
              child: _scanning
                  ? TextButton(
                      key: const ValueKey<String>('agents-pairing-use-code'),
                      onPressed: () => setState(() {
                        _scanning = false;
                        _message = null;
                      }),
                  ...
                  : TextButton(
                      key: const ValueKey<String>('agents-pairing-use-camera'),
                      onPressed:
                          (widget.cameraAvailable ?? agentsCameraIsDefault())
                          ? () => setState(() {
                              _scanning = true;
                              _message = null;
                            })
  ...
  Widget _buildCodeForm() {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: <Widget>[
          Text(
            'Your computer shows a code when Agents runs on it. Type or paste '
            'it here.',
            style: theme.textTheme.bodyLarge,
          ),
          const SizedBox(height: 16),
          TextField(
            key: const ValueKey<String>('agents-pairing-code-field'),
            controller: _code,
            autofocus: true,
            autocorrect: false,
          ...
          const SizedBox(height: 16),
          FilledButton(
            key: const ValueKey<String>('agents-pairing-connect'),
    ...
    return switch (error.errorCode) {
      MobileScannerErrorCode.permissionDenied =>
        'This app is not allowed to use the camera. Enter the code instead, '
            'or turn the camera on for Agents in your phone settings.',
