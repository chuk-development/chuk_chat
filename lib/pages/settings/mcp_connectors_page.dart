// lib/pages/settings/mcp_connectors_page.dart
//
...
class _McpConnectorsPageState extends State<McpConnectorsPage> {
  ...
  Future<void> _addByUrl() async {
    // The dialog owns its text controller in a StatefulWidget, so the controller
    // is disposed only after the dialog route is gone. Disposing it here, right
    // after showDialog returns, would free it while the exit animation still
    // paints the field, which throws "used after disposed".
...
Future<T> _withProgress<T>(
  ...
  messenger.showSnackBar(
    SnackBar(
      ...
      // Stay up for the whole connect so its Cancel stays reachable.
...
/// The "Add a connector" dialog. It owns its text controller so the controller
/// lives as long as the dialog route and is freed only in [dispose], after the
/// route is gone. Pops the entered URL, or null on cancel.
