  ...
  static Future<FileValidationResult> validateFile(String filePath) async {
    try {
      ...
      if (expectedMimeTypes != null && mimeType != null) {
        ...
        final isTextFile = expectedMimeTypes.contains('text/plain') ||
            mimeType.startsWith('text/');

        if (!isTextFile && !expectedMimeTypes.contains(mimeType)) {
          // Check if it's a generic type that might be acceptable
          final isAcceptable = mimeType == 'application/octet-stream' ||
              mimeType == 'text/plain' ||
              mimeType.startsWith('text/');
      ...
      return FileValidationResult.error('File validation error: ${e.toString()}');
    }
  }
  ...
  static Future<FileValidationResult> _validateArchive(File file) async {
    try {
      ...
      if (totalUncompressedSize > maxArchiveUncompressedSize) {
        final sizeMB = (totalUncompressedSize / (1024 * 1024)).toStringAsFixed(2);
        final maxMB = (maxArchiveUncompressedSize / (1024 * 1024)).toStringAsFixed(0);
        return FileValidationResult.error(
      ...
      return FileValidationResult.error('Failed to validate archive: ${e.toString()}');
    }
  }
