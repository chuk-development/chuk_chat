...
///    one (through [AgentsChatStore.replaceThread], so it also reaches the
...
class AgentsChatCacheMigration {
  ...
  @visibleForTesting
  static AgentsThreadWriter? writer;
    ...
    try {
      for (final file in await _jsonFiles(dir)) {
        try {
          ...
          final write = writer ?? AgentsChatStore.replaceThread;
          ...
          if (kDebugMode) {
            debugPrint(
              '[agents-chat-migration] $id: ${rows.length} rows from '
          ...
          if (kDebugMode) {
            debugPrint('[agents-chat-migration] ${file.path}: $error');
          }
        }
      }
    } catch (error) {
      if (kDebugMode) debugPrint('[agents-chat-migration] $error');
    }
    return migrated;
  }
  ...
  static Future<int> dropOrphanCursors(String userId) async {
    ...
    try {
      ...
      for (final key in prefs.getKeys().toList()) {
        ...
        if (kDebugMode) {
          debugPrint(
            '[agents-chat-migration] $sessionKey: no local copy, cursor '
            'dropped; the next open replays the whole thread',
          );
        }
      }
    } catch (error) {
      if (kDebugMode) debugPrint('[agents-chat-migration] cursors: $error');
    }
    return dropped;
  }
  ...
  static Future<bool> _hasLocalCopy(String userId, String sessionKey) async {
    ...
    return AgentsChatStore.hasThread(sessionKey);
  }
  ...
  static Future<void> _markMigrated(File file) async {
    try {
      ...
      if (kDebugMode) {
        debugPrint('[agents-chat-migration] rename failed: $error');
      }
    }
  }
}
