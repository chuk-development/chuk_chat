...
class OAuthLoopbackServer {
  ...
  Future<void> start({required String expectedState}) async {
    ...
    _server!.listen(
      _handle,
      // A socket-level failure never reaches _handle, so without this the
      // completer would never settle and the flow would hang.
      onError: _fail,
    );
  }
  ...
  Future<void> _handle(io.HttpRequest request) async {
    if (request.uri.path != '/callback') {
      // The result page makes the browser ask for /favicon.ico. Answering it
      // keeps that connection from staying open until stop() runs.
    ...
    if (code == null) {
      // Deliberately does NOT settle the completer: a stray or partial
      // callback must not end the run, so a later correct one still wins.
      // See oauth_loopback_server_test.dart.
