/// Supabase-backed encrypted mirror of the CoWork trust record.
///
/// This is the other half of "reinstall anywhere, reconnect automatically"
/// (see `docs/PRODUCT_PHILOSOPHY.md`). The local [CoworkPairingStore] keeps the
...
class SupabasePairingSync {
  ...
  Future<CoworkStoredPairing?> loadEncryptedPairing() async {
    try {
      ...
      return CoworkStoredPairing.tryParse(plaintext);
    } catch (error) {
      if (kDebugMode) {
        debugPrint('⚠️ [CoworkPairingSync] load returned null: $error');
      }
      return null;
    }
  }
  ...
  Future<void> clearEncryptedPairing() async {
    try {
      ...
      if (kDebugMode) {
        debugPrint('⚠️ [CoworkPairingSync] clear skipped: $error');
      }
    }
  }
