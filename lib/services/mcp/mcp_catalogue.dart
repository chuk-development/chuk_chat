...
/// Catalogue ids whose real brand logo ships in the binary as
/// `assets/mcp_icons/<id>.png`. Kept as an explicit set because assets cannot
/// be stat-ed at runtime: [bundledIconAsset] returns a path only for an id in
/// here, so a missing file never becomes a broken `Image.asset`.
///
/// Generated by `scripts/_dl_mcp_icons.sh` from the catalogue; every id below
/// downloaded a valid PNG. Registry and Add-by-URL servers are never here —
/// they keep the runtime favicon path.
const Set<String> kBundledMcpIcons = {
  'airtable',
  'asana',
  'atlassian',
  'box',
...
/// The bundled logo path for [id], or null when no logo ships for it. The
/// caller renders `Image.asset` when this is non-null and falls back to the
/// network favicon otherwise, so a registry or hand-added server (never in
/// [kBundledMcpIcons]) keeps working exactly as before.
String? bundledIconAsset(String id) =>
    kBundledMcpIcons.contains(id) ? 'assets/mcp_icons/$id.png' : null;
