...
class McpTool {
  ...
  /// The tool's JSON-Schema parameters, exactly as the server published them.
  ///
  /// RESTORED from upstream (`services/mcp/mcp_client.dart`, same class name and
  /// same `'inputSchema'` wire key). `services/mcp/mcp_tool_bridge.dart` hands
  /// it to `ClientTool.parameters`; without it every MCP tool reaches the model
  /// with no arguments at all. Defaulted so the Agents call sites that build a
  /// tool from name + description keep compiling, and `fromJson` tolerates a
  /// stored mirror row written before the field existed.
