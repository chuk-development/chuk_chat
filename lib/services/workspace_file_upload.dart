...
class WorkspaceUploadOutcome {
  ...
  /// The user closed the file picker, or declined the size confirmation.
...
/// picker that threw.
///
/// [confirmBytes] runs once the file is read and before anything is sent, so
/// a caller with a context budget can show the user what this file would cost
/// and let them back out. Return false to cancel.
Future<WorkspaceUploadOutcome> pickAndUploadWorkspaceFile({
  ...
  Future<bool> Function(int byteCount)? confirmBytes,
}) async {
  try {
    ...
    if (confirmBytes != null && !await confirmBytes(fileBytes.length)) {
      return const WorkspaceUploadOutcome.cancelled();
    }
