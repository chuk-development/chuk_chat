...
/// `agents_devices` table) are deliberately somebody else's problem.
class AgentsDeviceKeys {
  const AgentsDeviceKeys._();
  ...
  /// The public key, safe to publish to `agents_devices`.
  static Future<String> exportPublicKeyBase64(SimpleKeyPair keyPair) async {
    final publicKey = await keyPair.extractPublicKey();
    return base64Encode(publicKey.bytes);
  }

  /// Parses a peer public key received from `agents_devices`.
  ///
