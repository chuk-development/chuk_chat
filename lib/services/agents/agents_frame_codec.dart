...
import 'package:chuk_chat/services/agents/agents_replay_guard.dart';
...
/// Length in bytes of a raw Agents channel key (an AES-256 key).
///
...
const int kAgentsChannelKeyLength = 32;

/// Seals outgoing Agents frames: AES-256-GCM under the account key, then an
...
class AgentsFrameSealer {
  ...
  /// key. This is the Agents control-channel path (§14): the frame layout is
  ...
  factory AgentsFrameSealer.withChannelKey({
    ...
    if (channelKey.length != kAgentsChannelKeyLength) {
      throw ArgumentError.value(
        channelKey.length,
        'channelKey',
        'must be $kAgentsChannelKeyLength bytes',
      );
    }
    return AgentsFrameSealer(
  ...
  Future<AgentsFrame> seal(List<int> plaintext) async {
    final seq = _nextSeq++;
    final ts = _clock().toUtc().millisecondsSinceEpoch;
    final nonce = Uint8List.fromList(
      List<int>.generate(kAgentsFrameNonceLength, (_) => _random.nextInt(256)),
    );
    ...
    final header = AgentsFrame.buildHeaderBytes(
      version: kAgentsFrameVersion,
    ...
    final signature = await AgentsDeviceKeys.algorithm.sign(
      AgentsFrame.buildSignedBytes(
        header: header,
        nonce: nonce,
  ...
  Future<AgentsFrame> sealText(String plaintext) =>
      seal(utf8.encode(plaintext));
}

/// Opens incoming Agents frames, in this order:
///
...
class AgentsFrameOpener {
  AgentsFrameOpener({
    ...
    Map<String, int>? lastSeqByDevice,
       ...
       _clock = clock ?? DateTime.now,
       _guards = <String, AgentsReplayGuard>{
  ...
  /// key — the Agents control-channel path (§14). The verify/replay/decrypt
  /// pipeline is unchanged; only the AES key differs. [keyVersion] is the `kv`
  /// this receiver holds.
  factory AgentsFrameOpener.withChannelKey({
    required List<int> channelKey,
    required int keyVersion,
    required AgentsApprovedDevices approvedDevices,
    Duration replayWindow = const Duration(seconds: 60),
  ...
  final DateTime Function() _clock;
  final Map<String, AgentsReplayGuard> _guards;

  /// The live local trust store. Revoking here takes effect on the next frame.
  AgentsApprovedDevices get approvedDevices => _approvedDevices;
  ...
  /// Verifies and decrypts [frame], or throws [AgentsFrameRejectedException].
  Future<Uint8List> open(AgentsFrame frame) async {
    if (frame.version != kAgentsFrameVersion) {
      throw const AgentsFrameRejectedException(
        AgentsFrameRejection.unsupportedVersion,
      );
    }
    ...
    if (publicKey == null) {
      _logReject(AgentsFrameRejection.deviceNotApproved);
      throw const AgentsFrameRejectedException(
        AgentsFrameRejection.deviceNotApproved,
      );
    }

    // 2. Signature, before decryption.
    final signatureValid = await AgentsDeviceKeys.algorithm.verify(
      frame.signedBytes,
      signature: Signature(frame.sig, publicKey: publicKey),
    );
    if (!signatureValid) {
      _logReject(AgentsFrameRejection.badSignature);
      throw const AgentsFrameRejectedException(
        AgentsFrameRejection.badSignature,
      );
    }
    ...
    if (frame.keyVersion != _accountKeyVersion) {
      _logReject(AgentsFrameRejection.keyVersionMismatch);
      throw const AgentsFrameRejectedException(
        AgentsFrameRejection.keyVersionMismatch,
      );
    }
    ...
    final guard = _guards.putIfAbsent(
      frame.deviceId,
      () => AgentsReplayGuard(window: _replayWindow, clock: _clock),
    );
    try {
      guard.check(frame);
    } on AgentsFrameRejectedException catch (e) {
      _logReject(e.rejection);
      rethrow;
    }

    // 5. Decrypt. The header must match bit for bit or the tag fails.
    final cipherTextLength = frame.ciphertext.length - kAgentsFrameMacLength;
    ...
    try {
      ...
      _logReject(AgentsFrameRejection.decryptionFailed);
      throw const AgentsFrameRejectedException(
        AgentsFrameRejection.decryptionFailed,
      );
    }
  ...
  Future<String> openText(AgentsFrame frame) async =>
  ...
  void _logReject(AgentsFrameRejection rejection) {
    if (kDebugMode) {
      debugPrint('[Agents] frame rejected: ${rejection.name}');
    }
  }
}
