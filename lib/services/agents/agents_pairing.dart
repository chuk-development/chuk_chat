/// Agents secure pairing — SAS-authenticated X25519 with a hash commitment.
///
/// Dart twin of `common/agents_crypto/src/agents_crypto/pairing.py`. Implements
...
/// [AgentsPairingState.aborted] and discards key material.
class AgentsPairingException implements Exception {
  AgentsPairingException(this.rejection, [this.detail]);

  final AgentsPairingRejection rejection;
  final String? detail;

  @override
  String toString() => detail == null
      ? 'AgentsPairingException(${rejection.name})'
      : 'AgentsPairingException(${rejection.name}: $detail)';
}

/// Byte-exact protocol constants + pure crypto helpers, shared by both roles and
/// by the cross-language vector test.
class AgentsPairingCrypto {
  const AgentsPairingCrypto._();
...
///
/// Construct with [AgentsPairing.initiator] (Python client) or
/// [AgentsPairing.joiner] (desktop app). Each message-handling method consumes a
...
class AgentsPairing {
  AgentsPairing._({
    required AgentsPairingRole role,
  ...
  final AgentsApprovedDevices _approved;

  int _expiresAtMs;
  AgentsPairingState _state = AgentsPairingState.created;
  ...
  static Future<AgentsPairing> initiator({
    required String deviceId,
    required SimpleKeyPair deviceKeyPair,
    int sasDigits = AgentsPairingCrypto.defaultSasDigits,
    int expiryMs = AgentsPairingCrypto.defaultExpiryMs,
    int Function()? nowMs,
    ...
    return AgentsPairing._(
      ...
      nowMs: now,
      approvedDevices: approvedDevices ?? AgentsApprovedDevices.empty(),
    );
  }
  ...
  /// [onCommit].
  static Future<AgentsPairing> joiner({
    required String deviceId,
    required SimpleKeyPair deviceKeyPair,
    required String pairingCode,
    int Function()? nowMs,
    ...
    if (dash <= 0 || dash >= pairingCode.length - 1) {
      throw AgentsPairingException(
        AgentsPairingRejection.malformed,
        'pairing code',
      );
    }
  ...
  AgentsPairingRole get role => _role;

  AgentsPairingState get state => _state;
  ...
  String get sas {
    ...
    if (value == null) {
      throw AgentsPairingException(
        AgentsPairingRejection.wrongState,
        'SAS not derived yet',
      );
    }
    return value;
  ...
  AgentsApprovedDevices get approvedDevices => _approved;

  String? get peerDeviceId => _peerDeviceId;

  /// The 32-byte Agents channel key — **only** after both confirmation MACs
  ...
  Uint8List get channelKey {
    ...
    if ((_state == AgentsPairingState.confirmed ||
            _state == AgentsPairingState.completed) &&
        key != null) {
  ...
  void _require(AgentsPairingState expected) {
    if (_state == AgentsPairingState.completed ||
        _state == AgentsPairingState.aborted) {
      throw AgentsPairingException(
        AgentsPairingRejection.consumed,
        _state.name,
      );
    }
    if (_state != expected) {
  ...
  void _checkNotExpired() {
    if (_expiresAtMs != 0 && _nowMs() > _expiresAtMs) {
      _abort();
      throw AgentsPairingException(AgentsPairingRejection.expired);
    }
  }
  ...
  Map<String, dynamic> createCommit() {
    if (_role != AgentsPairingRole.initiator) {
      throw AgentsPairingException(
        AgentsPairingRejection.wrongState,
        'initiator only',
      );
    }
    _require(AgentsPairingState.created);
    _checkNotExpired();
    final msg = <String, dynamic>{
      'type': 'commit',
      'channel_id': _channelId,
      'commitment': base64.encode(
        AgentsPairingCrypto.commitment(_ephemeralPublic),
      ),
      'expires_at': _expiresAtMs,
      'sas_digits': _sasDigits,
    };
    _state = AgentsPairingState.commitSent;
    return msg;
  }
  ...
  Future<Map<String, dynamic>> onPubkey(Map<String, dynamic> msg) async {
    ...
    _require(AgentsPairingState.commitSent);
    _checkNotExpired();
    if (msg['type'] != 'pubkey') {
      throw AgentsPairingException(
        AgentsPairingRejection.malformed,
    ...
    _state = AgentsPairingState.keyEstablished;
  ...
  Future<Map<String, dynamic>> onConfirmD(Map<String, dynamic> msg) async {
    ...
    _require(AgentsPairingState.keyEstablished);
    _checkNotExpired();
    ...
    await _verifyConfirm(msg, AgentsPairingCrypto.confirmDLabel);
    _state = AgentsPairingState.confirmed;
    final mac = await AgentsPairingCrypto.deriveConfirmMac(
      _kRaw!,
      AgentsPairingCrypto.confirmCLabel,
      _currentTranscript(),
      _pairingCode,
    );
  ...
  void onCommit(Map<String, dynamic> msg) {
    if (_role != AgentsPairingRole.joiner) {
      throw AgentsPairingException(
        AgentsPairingRejection.wrongState,
        'joiner only',
      );
    ...
    if (msg['channel_id'] != _channelId) {
      throw AgentsPairingException(AgentsPairingRejection.channelMismatch);
    }
    final received = _decodeBytes(msg['commitment'], 'commitment');
    if (received.length != AgentsPairingCrypto.commitmentLength) {
      throw AgentsPairingException(
        AgentsPairingRejection.malformed,
        'commitment length',
      );
    ...
    _state = AgentsPairingState.commitReceived;
  }
  ...
  Map<String, dynamic> createPubkey() {
    ...
    _require(AgentsPairingState.commitReceived);
    _checkNotExpired();
    _state = AgentsPairingState.pubkeySent;
    return <String, dynamic>{
      'type': 'pubkey',
      'pubkey': base64.encode(_ephemeralPublic),
    };
  ...
  Future<Map<String, dynamic>> onReveal(Map<String, dynamic> msg) async {
    ...
    _require(AgentsPairingState.pubkeySent);
    _checkNotExpired();
    if (msg['type'] != 'reveal') {
      throw AgentsPairingException(
        AgentsPairingRejection.malformed,
    ...
    if (!AgentsPairingCrypto.constantTimeEquals(
      AgentsPairingCrypto.commitment(peerA),
      _commitment!,
    )) {
      _abort();
      throw AgentsPairingException(AgentsPairingRejection.commitmentMismatch);
    }
    _peerPublic = peerA;
    ...
    final mac = await AgentsPairingCrypto.deriveConfirmMac(
      _kRaw!,
      AgentsPairingCrypto.confirmDLabel,
      _currentTranscript(),
      _pairingCode,
    );
  ...
  Future<void> onConfirmC(Map<String, dynamic> msg) async {
    ...
    await _verifyConfirm(msg, AgentsPairingCrypto.confirmCLabel);
    _state = AgentsPairingState.confirmed;
  }
  ...
  void confirmPeerSas(String peerSas) {
    ...
    if (!AgentsPairingCrypto.constantTimeEquals(
      ...
      _abort();
      throw AgentsPairingException(AgentsPairingRejection.sasMismatch);
    }
  }
  ...
  /// Callable once the confirmation MACs verified ([AgentsPairingState.confirmed]).
  Future<Map<String, dynamic>> createDeviceKey() async {
    _require(AgentsPairingState.confirmed);
    _checkNotExpired();
    if (_sentDeviceKey) {
      throw AgentsPairingException(
        AgentsPairingRejection.wrongState,
    ...
    final label = _role == AgentsPairingRole.joiner
        ? AgentsPairingCrypto.deviceDLabel
        : AgentsPairingCrypto.deviceCLabel;
    final signature = await AgentsPairingCrypto.ed25519.sign(
      AgentsPairingCrypto.deviceProofMessage(t, _deviceId),
      keyPair: _deviceKeyPair,
    );
    final mac = await AgentsPairingCrypto.deviceMac(
      _kRaw!,
      label,
      t,
      edPub,
  ...
  Future<void> onPeerDeviceKey(Map<String, dynamic> msg) async {
    ...
    if (edPubRaw.length != AgentsDeviceKeys.publicKeyLength) {
      throw AgentsPairingException(
        AgentsPairingRejection.malformed,
        'ed25519_pub length',
      );
    ...
    final label = _role == AgentsPairingRole.joiner
        ? AgentsPairingCrypto.deviceCLabel
        : AgentsPairingCrypto.deviceDLabel;
    final expectedMac = await AgentsPairingCrypto.deviceMac(
      _kRaw!,
      label,
      t,
      edPubRaw,
    ...
    if (!AgentsPairingCrypto.constantTimeEquals(mac, expectedMac)) {
      _abort();
      throw AgentsPairingException(AgentsPairingRejection.macMismatch);
    }

    final peerEd = SimplePublicKey(edPubRaw, type: KeyPairType.ed25519);
    final validSignature = await AgentsPairingCrypto.ed25519.verify(
      AgentsPairingCrypto.deviceProofMessage(t, peerDeviceId),
  ...
  List<int> _currentTranscript() {
    ...
    return _role == AgentsPairingRole.initiator
        ? AgentsPairingCrypto.transcript(_ephemeralPublic, peer)
        : AgentsPairingCrypto.transcript(peer, _ephemeralPublic);
  }
  ...
  Future<void> _establishKey({
    ...
    final shared = await AgentsPairingCrypto.x25519.sharedSecretKey(
      keyPair: _ephemeralKeyPair,
      remotePublicKey: peerPublic,
    );
    final kRaw = await shared.extractBytes();
    _kRaw = kRaw;
    _channelKey = await AgentsPairingCrypto.deriveChannelKey(kRaw);
    final t = AgentsPairingCrypto.transcript(publicA, publicB);
    _sas = await AgentsPairingCrypto.deriveSas(
      kRaw,
      t,
      _pairingCode,
      _sasDigits,
  ...
  Future<void> _verifyConfirm(Map<String, dynamic> msg, String label) async {
    final mac = _decodeBytes(msg['mac'], 'mac');
    final expected = await AgentsPairingCrypto.deriveConfirmMac(
      _kRaw!,
      label,
      _currentTranscript(),
      _pairingCode,
    );
    if (!AgentsPairingCrypto.constantTimeEquals(mac, expected)) {
      _abort();
      throw AgentsPairingException(AgentsPairingRejection.macMismatch);
    }
  }
  ...
  List<int> _decodeKey(Object? value, String field) {
    ...
    if (raw.length != AgentsPairingCrypto.x25519PublicLength) {
      throw AgentsPairingException(
        AgentsPairingRejection.malformed,
        '$field length',
      );
