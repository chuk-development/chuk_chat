...
class BrowserPresence extends ValueNotifier<bool> {
  BrowserPresence(this.controller, {this.freshFor = kBrowserPresenceFreshness})
    : super(false) {
    final AgentsRelayState state = controller.state.value;
  ...
  /// (`mcp__<server>__<tool>`, `agents_agent.mcp_client.tool_name`).
  ...
  static String effectiveName(AgentsRelayTool tool) {
  ...
  static bool? stateFromTool(AgentsRelayTool tool) {
  ...
  static bool? stateFromView(AgentsRelayBrowserView view) {
  ...
  void _onInbound(AgentsRelayInbound event) {
    ...
    final bool? next = switch (event) {
      ...
      AgentsRelayTool() => stateFromTool(event) == false ? false : null,
      AgentsRelayBrowserView() => stateFromView(event),
      ...
      // field, fail closed.
      AgentsRelayRunState() => event.browserOpen == true && event.vncAvailable,
      ...
      AgentsRelayDone() => null,
      _ => null,
    };
    if (next == false) {
      _revoke(because: event is AgentsRelayBrowserView ? event.reason : '');
      return;
    }
    if (next == true) {
      _keep(light: true);
