...
/// from. This is the Agents trust boundary.
///
/// ## Why this exists rather than a server flag
///
/// A device's approval state also lives in Supabase (`agents_devices`), which
...
class AgentsApprovedDevices {
  ...
  factory AgentsApprovedDevices.fromBase64Map(Map<String, String> entries) {
    final approved = <String, SimplePublicKey>{};
    for (final entry in entries.entries) {
      approved[entry.key] = AgentsDeviceKeys.publicKeyFromBase64(entry.value);
    }
    return AgentsApprovedDevices(approved);
  }
  ...
  void approve(String deviceId, SimplePublicKey publicKey) {
    ...
    if (publicKey.type != KeyPairType.ed25519 ||
        publicKey.bytes.length != AgentsDeviceKeys.publicKeyLength) {
      throw ArgumentError.value(
        publicKey,
        'publicKey',
        'must be a ${AgentsDeviceKeys.publicKeyLength}-byte Ed25519 key',
      );
    }
