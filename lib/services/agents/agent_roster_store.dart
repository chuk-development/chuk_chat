...
class AgentRosterStore {
  ...
  List<AgentsAgent> _agents = const <AgentsAgent>[];
        ...
        if (decoded is Map) {
          ...
          if (rows is List) {
            for (final dynamic row in rows) {
              ...
              final AgentsAgent? agent = _readAgent(
          ...
          _agents = List<AgentsAgent>.unmodifiable(agents);
  ...
  void saveRoster(Iterable<AgentsAgent> agents, Set<String> hidden) {
    _agents = List<AgentsAgent>.unmodifiable(agents);
    _hidden = Set<String>.unmodifiable(hidden);
    _schedule();
  }
    ...
    try {
      ...
      final Map<String, dynamic> out = <String, dynamic>{
        'agents': <Map<String, dynamic>>[
          for (final AgentsAgent agent in _agents)
            <String, dynamic>{
              'id': agent.id,
              'name': agent.name,
              'onHost': agent.onHost,
  ...
  static AgentsAgent? _readAgent(Map<String, dynamic> row) {
    final Object? id = row['id'];
    if (id is! String || id.isEmpty) return null;
    final Object? name = row['name'];
    final List<AgentsThreadInfo> threads = <AgentsThreadInfo>[];
    ...
    if (threads.isEmpty) {
      ...
      threads.add(AgentsThreadInfo(key: id, title: 'General'));
    }
    return AgentsAgent(
      id: id,
