...
///    without a rebuild. Defaults to [kDefaultAgentsRelayBase].
///
...
class AgentsPairingInvite {
  const AgentsPairingInvite({
  ...
  /// `AgentsPairing` joiner and the host initiator both derive their SAS from.
  ...
  Uri toUri() => Uri(
    scheme: kAgentsPairingUriScheme,
    host: kAgentsPairingUriHost,
    queryParameters: <String, String>{
      'c': pairingChannel,
      'k': codeDigits,
      if (relayBase.toString() != kDefaultAgentsRelayBase)
  ...
  static AgentsPairingInvite? tryParse(String raw) {
    final text = raw.trim();
  ...
  static String? _extractLink(String text) {
    final match = RegExp(
      '(?:$kAgentsPairingUriScheme://|https?://)[^\\s\'"<>]+',
      caseSensitive: false,
    ).firstMatch(text);
    return match?.group(0);
  }

  static AgentsPairingInvite? _fromUri(String text) {
    final Uri uri;
    try {
      uri = Uri.parse(text);
    } on FormatException {
    ...
    if (uri.scheme.toLowerCase() == kAgentsPairingUriScheme) {
      final target = uri.host.isNotEmpty
          ? uri.host
          : uri.path.replaceAll('/', '');
      if (target.toLowerCase() != kAgentsPairingUriHost) return null;
    }
  ...
  /// exactly how `AgentsRelayClient.channelIdOf` reads it. A typed code and a
  /// scanned QR therefore resolve to the same channel and the same code.
  static AgentsPairingInvite? _fromBareCode(String text) {
  ...
  static Uri? _relayBaseFrom(String? raw) {
    final text = raw?.trim() ?? '';
    if (text.isEmpty) return Uri.parse(kDefaultAgentsRelayBase);
  ...
  @override
  String toString() => 'AgentsPairingInvite(relay: $relayBase)';
}
