...
///
/// ONE mapping for both paths: the live ledger ([AgentsRunLedger.recordTool])
...
/// [AgentsRelayDone.wasStopped]) or a failure, and either way the thread has to
/// say so instead of leaving the reader with the working dots.
AgentsRunOutcome agentsRunOutcomeFor({String? reason, String? finalAnswer}) {
  if (finalAnswer != null && finalAnswer.trim().isNotEmpty) {
    return AgentsRunOutcome.answered;
  }
  if (reason == 'estop' || reason == 'interrupted' || reason == 'timeout') {
    return AgentsRunOutcome.stopped;
  }
  ...
  if (reason == null || reason == 'finished') return AgentsRunOutcome.answered;
  return AgentsRunOutcome.failed;
}
...
String? agentsRunEndNotice(AgentsRunOutcome outcome) => switch (outcome) {
  AgentsRunOutcome.answered => null,
...
class AgentsRun {
  ...
  /// True between [AgentsRunLedger.begin] and [AgentsRunLedger.finish].
  bool running = false;
  ...
  /// by any sign of life; [AgentsRunLedger.ceilingGrace] after it, with still
  ...
  bool get endedWithoutAnswer =>
      !running &&
      outcome != null &&
      outcome != AgentsRunOutcome.answered &&
      !producedOutput &&
...
ContentBlock artifactBlockFromFile(String storagePath, AgentsRelayFile file) {
...
class AgentsRunLedger extends ChangeNotifier {
  ...
  static final AgentsRunLedger instance = AgentsRunLedger._();

  final Map<String, AgentsRun> _runs = <String, AgentsRun>{};
    ...
    for (final run in _runs.values.toList(growable: false)) {
      ...
      if (run.probedAt != null) {
        if (clock.difference(run.probedAt!) >= ceilingGrace) {
          _endWithoutAnswer(run.sessionKey, AgentsRunOutcome.lost);
        }
        continue;
      }
      if (silent >= ceiling) {
  ...
  AgentsRun? runFor(String sessionKey) => _runs[sessionKey];
  ...
  AgentsRun begin(String sessionKey) {
    ...
    final run = AgentsRun(sessionKey)..running = true;
  ...
  void _endWithoutAnswer(String sessionKey, AgentsRunOutcome outcome) {
    final run = _runs[sessionKey];
    if (run == null || !run.running) return;
    run
      ..running = false
  ...
  void markLost(String sessionKey) =>
      _endWithoutAnswer(sessionKey, AgentsRunOutcome.lost);
  ...
  void reconcileIdle(String sessionKey) {
    ...
    _endWithoutAnswer(
      sessionKey,
      run.stopRequested ? AgentsRunOutcome.stopped : AgentsRunOutcome.lost,
    );
  }
  ...
  ToolCall automation(String sessionKey, AgentsRelayAutomation event) {
    final run = _live(sessionKey);
  ...
  /// produces no block — [AgentsRelayFile.error] already says why.
  Future<ContentBlock?> file(String sessionKey, AgentsRelayFile file) async {
  ...
  ToolCall approval(String sessionKey, AgentsRelayApprovalRequest request) {
    final run = _live(sessionKey);
  ...
  AgentsRun? take(String sessionKey) {
  ...
  AgentsRun _ensure(String sessionKey) =>
      _runs[sessionKey] ??= AgentsRun(sessionKey);
  ...
  AgentsRun _live(String sessionKey) {
    final run = _ensure(sessionKey);
    if (run.running) {
      run
        ..lastActivity = DateTime.now()
