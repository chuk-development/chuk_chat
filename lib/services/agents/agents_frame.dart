/// Agents wire frame: `{seq, ts, nonce, ciphertext, sig}`.
///
...
/// forge or replay one.
///
/// Layered defences, in the order `AgentsFrameOpener` applies them:
...
const String kAgentsFrameVersion = '1';

/// AES-GCM nonce length in bytes.
const int kAgentsFrameNonceLength = 12;

/// Ed25519 signature length in bytes.
const int kAgentsFrameSignatureLength = 64;

/// AES-GCM authentication tag length in bytes, appended to [AgentsFrame.ciphertext].
const int kAgentsFrameMacLength = 16;
...
enum AgentsFrameRejection {
  ...
  ///
  /// The Supabase `agents_devices` row is a UX convenience and a first-line
...
class AgentsFrameRejectedException implements Exception {
  const AgentsFrameRejectedException(this.rejection, [this.detail]);

  final AgentsFrameRejection rejection;
  ...
  String toString() => detail == null
      ? 'AgentsFrameRejectedException(${rejection.name})'
      : 'AgentsFrameRejectedException(${rejection.name}: $detail)';
}
...
class AgentsFrame {
  AgentsFrame({
    ...
    required this.sig,
    this.version = kAgentsFrameVersion,
    this.keyVersion = 1,
  });
  ...
  /// AES-GCM nonce, [kAgentsFrameNonceLength] random bytes, unique per frame.
  final Uint8List nonce;

  /// AES-GCM output: cipher text followed by the [kAgentsFrameMacLength]-byte tag.
  ...
  /// not a well-formed frame throws [AgentsFrameRejection.malformed] rather
  /// than reaching the cryptographic layer with a surprising shape.
  factory AgentsFrame.fromJson(Map<String, dynamic> json) {
    ...
    if (version != kAgentsFrameVersion) {
      throw const AgentsFrameRejectedException(
    ...
    if (nonce.length != kAgentsFrameNonceLength) {
      throw const AgentsFrameRejectedException(
        AgentsFrameRejection.malformed,
        'nonce length',
      );
    ...
    if (ciphertext.length < kAgentsFrameMacLength) {
      throw const AgentsFrameRejectedException(
        AgentsFrameRejection.malformed,
    ...
    if (sig.length != kAgentsFrameSignatureLength) {
      throw const AgentsFrameRejectedException(
        AgentsFrameRejection.malformed,
        'sig length',
      );
  ...
  /// [AgentsFrameRejection.malformed], not a [FormatException] escaping into
  /// the relay read loop.
  factory AgentsFrame.fromJsonString(String source) {
    ...
    return AgentsFrame.fromJson(decoded);
  }

  static Uint8List _decodeBase64(Object? value, String field) {
    if (value is! String) {
      throw AgentsFrameRejectedException(
        AgentsFrameRejection.malformed,
