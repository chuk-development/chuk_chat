...
///    content blocks).
///  * [AgentsRunLedger] — the live run for the thread, for the raw
...
abstract final class ChatDebugExport {
  ...
  static Future<Map<String, dynamic>> build({required String threadKey}) async {
    ...
    final run = AgentsRunLedger.instance.runFor(threadKey);
  ...
  static Future<String> buildJson({required String threadKey}) async {
    try {
      ...
      if (kDebugMode) debugPrint('[agents-export] encode failed: $error');
  ...
  static Future<List<Map<String, dynamic>>> _rowsFor(String threadKey) async {
    try {
      ...
      if (kDebugMode) debugPrint('[agents-export] cache read failed: $error');
