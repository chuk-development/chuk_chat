-- Migration: Add tool-calling settings to customization_preferences
...
COMMENT ON COLUMN customization_preferences.tool_calling_enabled IS
  'Master toggle for client-side tool calling loop';

COMMENT ON COLUMN customization_preferences.tool_discovery_mode IS
  'Require find_tools discovery before invoking other tools';

COMMENT ON COLUMN customization_preferences.show_tool_calls IS
  'Display tool execution chips in assistant message bubbles';

COMMENT ON COLUMN customization_preferences.allow_markdown_tool_calls IS
