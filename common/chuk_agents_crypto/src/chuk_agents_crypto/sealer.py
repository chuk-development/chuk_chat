"""Seal and open Agents frames — Python twin of ``agents_frame_codec.dart``.

AES-256-GCM under the injected Agents **channel key** (see ``channel_key.py`` —
...
class AgentsFrameSealer:
    """Seals outgoing Agents frames. One sealer per outbound stream; it owns the
    ...
    storage rather than start over. Mirrors ``AgentsFrameSealer``."""
    ...
    def seal(self, plaintext: bytes) -> AgentsFrame:
    ...
    def seal_text(self, plaintext: str) -> AgentsFrame:
        return self.seal(plaintext.encode("utf-8"))


class AgentsFrameOpener:
    """Opens incoming Agents frames. Mirrors ``AgentsFrameOpener``."""
    ...
    def open(self, frame: AgentsFrame | bytes | str) -> bytes:
        """Verify and decrypt ``frame``, or raise ``AgentsFrameRejected``.

        Accepts a parsed :class:`AgentsFrame`, raw wire bytes, or the JSON text.
        """
        if isinstance(frame, (bytes, bytearray)):
            frame = AgentsFrame.from_bytes(bytes(frame))
        elif isinstance(frame, str):
            frame = AgentsFrame.from_json_string(frame)
        ...
        try:
            ...
            raise AgentsFrameRejected(AgentsFrameRejection.BAD_SIGNATURE)
        ...
        if frame.key_version != self._key_version:
            raise AgentsFrameRejected(AgentsFrameRejection.KEY_VERSION_MISMATCH)
        ...
        try:
            ...
            raise AgentsFrameRejected(AgentsFrameRejection.DECRYPTION_FAILED)
    ...
    def open_text(self, frame: AgentsFrame | bytes | str) -> str:
...
def seal(
    ...
    *,
    ...
    :class:`AgentsFrameSealer` for one-shot use."""
...
def open(
    ...
    *,
    ...
    ``AgentsFrameRejected``. A thin wrapper over :class:`AgentsFrameOpener`."""
