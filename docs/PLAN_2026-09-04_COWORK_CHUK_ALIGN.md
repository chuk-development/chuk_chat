# Plan: cowork = chuk_chat master, self-hosted backend

## Context

cowork is a fork of chuk_chat. Over time its chat UI, settings, and model selector
...
> model response it calls the self-hosted cowork Python backend.
...
`package:chuk_chat/` → `package:cowork/`, and change ONLY the line(s) that call
the hosted API. Nothing else is redesigned.

## The one integration seam (from the cowork exploration)

cowork does not talk HTTP. It opens one end-to-end-encrypted WebSocket to the
local Python host (`ws://127.0.0.1:8787`) via `CoworkRelayClient`, behind the
`CoworkRelayController` interface in
`app/lib/services/cowork/cowork_relay_client.dart`.
...
- receive: fold `controller.inbound` (`Stream<CoworkRelayInbound>`) into the chat's
  message model. The 15 inbound variants and the mapping to message entries are in
  `cowork_thread_view.dart` `_onInbound` (lines ~494–577) — this is the reference:

| Relay event | becomes |
|---|---|
| `CoworkRelayDelta` (text, replay) | assistant answer token → accumulate in current assistant message |
| `CoworkRelayReasoning` (text) | reasoning/thinking block (process detail) |
| `CoworkRelayTool` (name, status, arguments, result, exitCode, failed, timedOut, duration, replay) | tool-call line (process detail) |
| `CoworkRelayFile` (name, mimeType, bytes, error) | file/attachment card |
| `CoworkRelaySubagent` (id, title, state, result, error) | subagent row, updated in place |
| `CoworkRelayUser` (text, replay=true) | user turn — only from server replay |
| `CoworkRelayDone` (finalAnswer, reason, iterations, tokensSpent, replay) | end of run; a replay `done` ends history only, not a run |
| `CoworkRelayRunError` (message) | error entry |
| `CoworkRelayApprovalRequest` | "publish to web?" card with decide buttons |
| `CoworkRelayDebugContext` | not rendered; kept for debug copy |
...
`CoworkThreadView` public constructor is what the shell drives; keep it:
...
- **State:** Riverpod (`ProviderScope`) + static singleton services with `ValueNotifier`s.
  cowork has NO Riverpod today (plain `ValueNotifier`/`setState`). To import the chat
  runtime verbatim, cowork adds `flutter_riverpod` and wraps in `ProviderScope`.
- **Shell layout (no AppBar):** `lib/platform_specific/root_wrapper_desktop.dart`
  ...
  **floating positioned icon buttons**. Top-left: hamburger + mini-rail (New chat,
  ...
  → This top-right slot is where cowork puts its own actions (Agents, Control
...
- **Left sidebar** = chat history (`sidebar_desktop.dart` `SidebarDesktop`,
  `sidebar_mobile.dart`; chrome in `widgets/sidebar/sidebar_chrome.dart`). In cowork
...
- **MCP OAuth runs ON THE DEVICE:** `lib/services/mcp/mcp_oauth.dart` (`McpOAuth`) +
  ...
  → cowork's recent MCP port moved OAuth "host-side" and nothing initiates it, so
...
- **Rooms / conference: does not exist in chuk_chat.** cowork's Control Rooms are
...
cowork runs the agent **server-side**: the Python agent executes tools autonomously and
streams progress. So `CoworkRelayTool` means "the server ran this tool, here is the
...
- **Ownership.** Mine: everything under `app/lib` and `app/test` EXCEPT
  ...
  `app/analysis_options.yaml` (cowork-13). Also mine: `agent/src/cowork_agent/mcp_client.py`,
...
Good news found while planning: cowork already holds **byte-identical** copies (modulo
...
Import verbatim (`package:chuk_chat/` → `package:cowork/`, no other edits):
...
- Widgets: `expressive_settings.dart` (take chuk's over cowork's near-copy),
...
- Do NOT import `sidebar_desktop.dart`/`sidebar_mobile.dart` verbatim (~90% chat-history
  ...
  Settings entry**, as in chuk; `SbBrand(label: 'CoWork')`. Data: `AgentRosterSource`.
...
- Single socket: new `app/lib/pages/cowork_shell_state.dart` mixin `CoworkShellHost`
...
| connectors | keep cowork's (move to `pages/mcp_connectors_page.dart`, chuk's path) |
| skills | import UI, hidden behind `kFeatureSkills` until a relay verb backs it |
| sandboxes | HIDE (hosted provisioning; cowork's sandbox is host-side) |
...
| developer | keep cowork's page under the same id; port `DeveloperOptionsService` |
| NEW cowork dests | `herenow`, `embedding`, `view` (verbose, searchable), `host` (host URL, pairing status, forget/re-pair) |
...
- Import verbatim into `services/mcp/`: `mcp_oauth.dart` (edit: `clientName='CoWork'`,
  ...
  verbatim into cowork's `McpService`.
...
`agent/src/cowork_agent/mcp_client.py`: (1) `configs_from_entries` keeps an OAuth entry
...
ownership unit). cowork's `models/chat_stream_event.dart` has DELETED `ToolCallsEvent` /
...
`theme_extensions` overwrite — chuk's `MaterialYouTokens` is a superset; keep cowork's
`kMenuIconColor` in `utils/cowork_theme_extras.dart`); misc (10 incl. `constants`,
`platform_config` overwrite + re-add cowork flags, `l10n/*`); assets (fonts, mic icon).
**EXCLUDE `services/mcp/*` from the import** — cowork's MCP (already verbatim-ported,
plus WS-4) is the source of truth; any symbol the chat UI needs from it is satisfied by
cowork's files or a thin stub, never an overwrite. `mcp_client.dart` (device MCP client)
is stubbed if referenced.

**Stubs (~25, cowork-owned, upstream-signature-compatible, fixed header comment
"COWORK STUB. Upstream: … @ <sha>. Reason: …"):** `websocket_chat_service.dart` (**THE
...
| `CoworkRelayInbound` (live) | `ChatStreamEvent` | `CoworkRunLedger` side-effect |
|---|---|---|
...
| Delta/User/Done with `replay:true` | — (never into a live run) | routed to `CoworkReplayLoader` |
...
**Locator — `services/cowork/cowork_relay_link.dart`:** singleton with
`ValueNotifier<CoworkRelayController?> controller`, `ValueNotifier<String> sessionKey`,
...
**`CoworkThreadView`:** constructor and lifecycle half unchanged (`_bootstrap`,
...
**History — server-authoritative:** `CoworkReplayLoader` (`services/cowork/`) folds
...
`tool_call_handler_stub_test` (`shouldContinue` always false), `cowork_run_ledger_test`,
`cowork_replay_loader_test` (second replay overwrites, no duplicates),
`cowork_thread_view_test` split: keep pairing/connect/reconnect half, rewrite the
...
- `host/notify.py` `SupabaseNotifier` (implements the existing unused
  `cowork_manager.autonomy.Notifier` seam): single-worker thread (never delays the task
  ...
  `mark_notified` only on success. Optional keyless sinks `COWORK_NTFY_TOPIC`,
  `COWORK_WEBHOOK_URL`.
- `host/desktop_notify.py` `DesktopNotifier`: `notify-send --app-name=CoWork
  --hint=string:desktop-entry:cowork …`, fallback `gdbus … Notify`, macOS `osascript`;
...
- **Anti-duplicate rule:** no controller attached → Supabase/FCM push + desktop; attached +
  ...
  `flutter_local_notifications` on the live `CoworkRelayDone` (the app knows its own
...
- App: port chuk's `notification_service_io.dart` → `services/notifications/local_notifications.dart`
  ...
  CoWork device id, `onTokenRefresh`, auth-state hooks, tap → `NotificationRouter`),
...
- **Reopen flow:** per-session replay cursor `lastMid` in SharedPreferences; ALWAYS
  ...
  `_RunPhase.detached` "Working… (reconnecting)"; handle `run_state`; `CoworkRelayDone`
...
3. `model_selector_page.dart`: imported VERBATIM (WS-3), not a thin push to cowork's page;
   cowork's `model_settings_page.dart` is deleted.
4. MCP: cowork's `services/mcp/*` is the source of truth (SC1's verbatim port + WS-4
...
6. Replay `done` semantics: `reason == 'replay'` is the history-end marker (renders
   ...
   real completion card. `CoworkReplayLoader` implements exactly this split.
...
`cowork` for long; merge behind a green gate.
...
| P1 | **Contract + relay client**: extend `cowork_relay_client.dart` (`CoworkRelayDone.runId/whileAway/isHistoryEnd`, `run_state` inbound, `requestReplay(afterId:)`, `run_ack`), restore `chat_stream_event.dart` + `multiplex_connection.dart` verbatim, `theme_extensions` overwrite + extras. | relay client tests green |
| P2 | **WS-6 Chat-UI import**: import script + manifest → 125 files → stubs until 0 errors (the big gate, budget 2–3× any other step) → `CoworkRunLedger` + `CoworkRelayLink` → adapter → `ToolCallHandler` stub → `CoworkReplayLoader` + local `ChatStorageService` → `CoworkThreadView` rewrite → 2-line shell wiring → `docs/CHAT_UI_IMPORT.md`. | 0 analyze errors; adapter/ledger/loader/stub tests + rewritten `cowork_thread_view_test` green; manual `flutter run -d linux`: task streams, tool card, file card, Stop, kill+reopen replays |
| P3 | **WS-1 Shell**: `CoworkShellHost`, root-wrapper layout (desktop+mobile), Agents sidebar on chuk chrome, right panel (rooms/browser), top-right 4 actions incl. Copy full chat, AppBar removed. | `messenger_shell_test` + ported roster tests + single-socket resize assertion |
...
- Branding: `McpOAuth.clientName = 'CoWork'`, plain-text `SbBrand('CoWork')`.
