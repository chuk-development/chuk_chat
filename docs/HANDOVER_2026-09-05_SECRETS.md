...
| Piece | Where | Notes |
|---|---|---|
| Tools + scrubber + access protocol | `agent/src/chuk_agents_runtime/secrets.py` (new) | `request_secrets(names, purpose)`, `list_secrets()`, `Scrubber` (raw / base64 / base64url / URL-encoded, values >= 8 chars), `SecretsAccess` protocol, `DictSecrets` for tests, `status_map`, `valid_name`. |
...
| Vault | `executor/src/chuk_agents_executor/secrets.py` (new) | `SecretsVault`: in-memory set, `replace(entries, revision)` (last frame wins), `env()`, `names()`, `status()`, `scrubber()`, at-rest AES-256-GCM file (0600) `load()`/`save()`. |
...
| Host | `host/host.py`, `host/serve.py`, `host/secrets_key.py` (new) | `LocalHost.secrets_vault` (built in `__init__`, `~/.agents/secrets.enc`, key = HKDF(identity seed, "cowork/host/secrets-at-rest/v1")), loaded at start; passed through `TaskServer(secrets=)`; `_on_secret_request_pending` → desktop toast (names only) when no app is attached. |
...
| Piece | Where |
|---|---|
| Store (secure storage, one record `agents_secrets_v1`, revision) | `app/lib/services/secrets/secrets_store.dart` |
...
| Relay | `agents_relay_client.dart`: `AgentsRelaySecretRequest`, `sendSecrets(values:, revision:, requestId:)`, ctor `secretsForwarder:` (called after every `provisionAccount`), `_dispatch` case `secret_request` |
| Thread view card | `agents_thread_view.dart`: `_buildSecretRequestBar` (one obscured field per name, "Already set" helper, Save keys / Skip) |
| Settings | `app/lib/pages/secrets_settings_page.dart` (new); hub rows in `settings_page.dart` (Agents section) and `desktop_settings_modal.dart` (`id: 'apikeys'`) |
| Wiring | `agents_shell_state.dart`, `session_recovery.dart`: `secretsForwarder: SecretsService.instance.forwardToHost` |
| Exhaustive switches | one `case AgentsRelaySecretRequest()` each in `websocket_chat_service.dart` and `agents_replay_loader.dart` |
| Migration + schema | `supabase/migrations/20260905120000_cowork_secrets.sql`, `docs/SUPABASE_SCHEMA.md` (section "Agents secrets") |
...
`secrets_service_test.dart`, `test/services/agents/agents_relay_secrets_test.dart`,
`test/pages/secrets_settings_page_test.dart`,
`test/widgets/agents_thread_view_secrets_test.dart`; `settings_page_test.dart`
...
- `agents_run_ledger.dart` untouched: a `secret_request` is not a transcript
...
- 2026-09-05 ~14:30 — Dart tests run one at a time in the coordinator's
  compiler window: `test/services/secrets` 13, `agents_relay_secrets_test`
  4, `secrets_settings_page_test` 6, `settings_page_test` 4,
  `agents_thread_view_secrets_test` 4, `agents_thread_view_test` 25
