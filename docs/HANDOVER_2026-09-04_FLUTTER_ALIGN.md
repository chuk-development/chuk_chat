# Handover 2026-09-04 — agents = chuk_chat master, self-hosted backend
...
2. `docs/PLAN_2026-09-04_AGENTS_CHUK_ALIGN.md` — the approved plan (P0–P8), the
...
agents IS chuk_chat master (`~/git/chuk_chat`, branch `master`). Same chat UI, same
...
`scripts/import_chat_ui.sh`. Do not fix (b) in agents directly.
...
| P2b Relay adapter, tool fold, run ledger, relay link, replay loader + cursor, local chat cache, `AgentsThreadView` body → `ChukChatUIDesktop/Mobile` | DONE, green (2026-09-05 ~01:40). Lib: full `flutter analyze` 0 errors; new unit tests green (websocket_chat_service 16, tool_call_handler_stub 7, agents_run_ledger 14, agents_replay_loader 12); widget harness fixed by cowork-5c (messenger_shell 15, agents_thread_view 22, widget_test 3; no assertion weakened). Live streaming proof against the host: pending the app restart | bead `cowork-4y5` closed |
| P3 Shell: chuk root-wrapper layout, Agents sidebar on chuk chrome, right panel (rooms/browser), top-right 4 actions incl. Copy full chat, AppBar removed, `AgentsShellHost` hoisted above the desktop/mobile switch | NOT STARTED | bead `cowork-b6h` |
...
Python (all committed on branch `agents`):
- `agent/src/chuk_agents_runtime/state.py` — `runs` table, `replay_events(after_id)` + `mid`,
  `begin_run/finish_run/fail_run/run_terminals/mark_run_notified/mark_run_seen/sweep_orphan_runs`.
- `executor/src/chuk_agents_executor/executor.py` — `rebind_codec`, `_codec_lock`,
...
- `executor/src/chuk_agents_executor/protocol.py` — `run_state_payload`, `run_ack_payload`,
  `replay_payload(after_id)`, `done_payload(run_id, while_away)`.
- `host/src/chuk_agents_host/party.py` — a controller leaving does NOT stop the task
...
- `host/src/chuk_agents_host/serve.py` — `rebind`, hook pass-through.
- `host/src/chuk_agents_host/host.py` — orphan-run sweep at start, `_on_reprovision`,
  `_on_run_finished` → `SupabaseNotifier`, `_user_id` from the token frame.
- `host/src/chuk_agents_host/desktop_notify.py` — `notify-send`/`gdbus`/`osascript`, on a
  daemon thread (it must never block the task worker — that was the second real
  bug). Tests set `AGENTS_DESKTOP_NOTIFY=0`.
- `host/src/chuk_agents_host/notify.py` — `SupabaseNotifier`: row with the user's own
...
- `lib/main.dart` — chuk bootstrap: `AppThemeService`, `DynamicColorBuilder`, l10n
  delegates, `_buildShellConfig()`, `AgentsApp.shellConfig` static (TODO P3: thread
...
- `lib/services/agents/agents_relay_client.dart` — the ONE seam to the backend.
  `AgentsRelayController`: `sendTask/requestStop/requestReplay(afterId)/sendRunAck/...`;
  inbound sealed variants incl. `AgentsRelayRunState`, `AgentsRelayDone.runId/whileAway/isHistoryEnd`,
...
- `lib/services/agents/agents_relay_link.dart`, `agents_run_ledger.dart`,
  `agents_replay_loader.dart` — P2b.
- `lib/services/chat_storage_service.dart` — local instant-paint cache only (never
  authoritative; the server replays history).
- `lib/widgets/agents_thread_view.dart` — keeps its public constructor and the
...
- `lib/services/mcp/**` — agents's MCP (54-server catalogue verbatim, `McpStore` with
...
- Kept on purpose, not overwritten: `services/websocket_connector_io.dart` (13's 20 s
  ping interval), `widgets/mcp_connect_card.dart` (agents's port).
...
`tools/chat_ui_manifest.txt`, `tools/platform_config_agents_extras.dart.part`,
`scripts/import_chat_ui.sh`, `docs/PRODUCT_PHILOSOPHY.md`, `docs/CHAT_UI_IMPORT.md`,
`docs/PLAN_2026-09-04_AGENTS_CHUK_ALIGN.md`, this file.
...
2. The remaining hang ("did not complete" after ~6:49 in 'an agent has one permanent
   ...
   `AgentsRelayDone` after `sendTask`, so the adapter's stream
   (`websocket_chat_service.dart`) never closes and the imported send logic waits
   forever. Fix in the tests: after every simulated send, emit a `AgentsRelayDone`
...
Check what exists: `agents_relay_link.dart`, `agents_run_ledger.dart`, the real
`websocket_chat_service.dart` and `tool_call_handler.dart`, `chat_storage_service.dart`
as a local cache were already written; `agents_replay_loader.dart`, the
`agents_thread_view.dart` rewrite, the two-line shell wiring and the new test files
(`test/services/websocket_chat_service_test.dart`,
`test/services/tool_call_handler_stub_test.dart`,
`test/services/agents/agents_run_ledger_test.dart`,
`test/services/agents/agents_replay_loader_test.dart`, rewritten
`test/widgets/agents_thread_view_test.dart`) may be missing. The full P2b brief is
...
(new, modelled on chuk's), `lib/pages/agents_shell_state.dart` (new mixin
`AgentsShellHost`), `lib/pages/messenger_shell.dart`, `lib/widgets/agent_roster_view.dart`,
...
- **c91 app side is DONE and green** (`agents_relay_client_test` 44/44):
  ...
  `expires_at`. Files: `lib/services/agents/agents_relay_client.dart`,
  `lib/services/account_session.dart`, `lib/services/executor_provisioning.dart`,
  `test/services/agents/agents_relay_client_test.dart`, `docs/WIRE_CONTRACT.md`
...
- **THE TREE DOES NOT COMPILE RIGHT NOW (≈65 analyze errors)** — caused by the
  ...
  from hosted-only chuk dependencies agents does not have:
  ...
  `pages/mcp_connectors_page.dart` path (agents's is `pages/settings/mcp_connectors_page.dart`),
  `DeveloperOptionsPage` (agents's is `pages/settings/developer_settings_page.dart`),
  plus methods the agents stubs lack (`TitleGenerationService.isEnabled/getSystemPrompt/
  ...
  agents's page, developer at agents's page, strip credits/profile/password rows
  from account, strip title-generation rows from customization, make the
  system-prompt page use a small agents store or drop it) — the eight files are the
  ...
  analyze was 0 errors, and agents's old settings hub
...
- `cowork-6v5`: "Encryption key is not available for the current user" at startup —
  chuk derives the EncryptionService key from the password at login; agents restores
...
- **cowork-5c**: P3 Shell (WS-1: root-wrapper layout, Agents sidebar on chuk
  ...
  button 5c already put in `messenger_shell.dart`, `AgentsShellHost` hoisted above
...
- **cowork-47 owns P5 MCP OAuth**: `lib/services/mcp/**`, `lib/pages/settings/mcp_connectors_page.dart`,
  `lib/widgets/mcp_connect_card.dart`, `agent/src/chuk_agents_runtime/mcp_client.py`,
...
- **cowork-5c also, right now**: the top-right "Copy full chat" button in
  ...
  `lib/services/agents/chat_debug_export.dart` (public static API) so the button can
...
- **This session keeps**: P2b (thread view, `websocket_chat_service.dart`,
  `tool_call_handler.dart`, `agents_run_ledger/relay_link/replay_loader.dart`,
  `chat_storage_service.dart`), P3 shell (`messenger_shell.dart`, root wrappers,
  sidebar, `agents_shell_state.dart`, `auth_gate.dart`), P4-rest (settings page +
...
| `lib/services/model_info_service.dart` | agents's own; loads `/v1/models_info` through the local relay (`ApiConfigService` already points there). Zero edits needed for the fetch path |
...
| `lib/pages/settings/model_settings_page.dart` | cowork-adapted, to be DELETED and replaced by `model_selector_page.dart`; `AgentsThreadView.onOpenModelScreen` re-points to the modal (desktop) / `ModelSelectorPage` (mobile) — same `VoidCallback` |
...
| `lib/services/mcp/mcp_store.dart` | agents's: SharedPreferences config + secure-storage secrets; `forwardPayloads()` → `[{name,url,auth,access_token?}]` rides `sendTask` as `mcp_servers`. P5 turns the secret into chuk's full record (`mcp_secrets_<id>`) and adds the `oauth` forward block (contract in the plan, WS-4) |
| `lib/services/mcp/mcp_connector_sync.dart` | agents's encrypted Supabase mirror (one blob per user, like `supabase_pairing_sync.dart`); P5 mirrors the full record |
| `lib/services/mcp/mcp_oauth.dart`, `mcp_redirect.dart`, `mcp_redirect_io.dart`, `mcp_redirect_stub.dart` | MISSING — port VERBATIM from chuk (edit only `clientName='Agents'`, `clientUri`, the redirect page copy) |
...
- a full `scripts/import_chat_ui.sh` run is safe **as long as chuk_chat still
  holds the fix in its working tree**: the Agents copy is byte-identical to
...
The test came along, deliberately, so Agents guards the behaviour itself:
...
| `customization_page.dart` | adapted: Downloads section and the whole auto-chat-title block (`TitleGenerationService` state, loaders, save/reset, prompt editor) removed; a Agents "Detail" section with the `VerboseService` full-log switch takes their place |
| `account_settings_page.dart` | **chuk's version discarded, not adapted.** It is 780 lines over `ProfileService`, `PasswordChangeService`, `PasswordResetService`, `KeyVersionService` and `RecoverChatsPage`, none of which exist here. Agents's own 67-line page already does the right thing (who is signed in, and the way out) and was moved to `pages/account_settings_page.dart` per the plan |
| `system_prompt_page.dart` | **dropped.** It needs `loadSoulText` / `saveSoulText` / `isIdentityEnabled` from chuk's `tool_handlers/notes_tools.dart`. The host owns the system prompt (docs/PRODUCT_PHILOSOPHY.md), so a Agents-side store would be a second source of truth. Coordinator decision, 2026-09-05 |
| `settings_page.dart` | adapted (1155 → 602 lines): hidden are pricing, identity, tool calling, sandboxes, Data & Privacy/export and the onboarding replay; connectors and developer point at Agents's own pages; a new "Agents" section carries here.now and Embedding; footer wordmark is Agents |
...
together with the deletion of Agents's old `pages/settings/settings_page.dart`,
