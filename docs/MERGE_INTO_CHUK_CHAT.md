# Merging CoWork into chuk_chat

CoWork and chuk_chat are two working trees of one product. They share no commit.
CoWork took the chat UI as a file copy (`scripts/import_chat_ui.sh`), so the same
...
the repository root, Dart package `chuk_chat`. The CoWork features behind the
`FEATURE_COWORK` dart-define that `scripts/build_apk.sh` already sets. The Python
...
|---|---|---:|---:|
| CoWork | `cowork` @ `869748c`, root `593fe9f` (2026-08-12 … 2026-09-13) | 306 | 1330 |
...
`git replace --graft 593fe9f d31526a` makes the CoWork root commit a child of the
upstream baseline. Because the graft computes the root commit's diff against the
full baseline tree, the root commit stops meaning "CoWork starts here" and starts
...
(`docs/COWORK_AGENT_PLATFORM_PLAN.md`). Rename detection adds noise rather than
sense: it pairs `vendor/markdraw/android/**` with `app/android/**` and pairs
`.codex` with `common/cowork_crypto/src/cowork_crypto/py.typed`.
...
CoWork decision, and applies them:
...
| Conflicts | 201 |
| … of type `modify/delete` (upstream changed a file CoWork "deleted") | 107 |
| … of type `content` | 62 |
...
The CoWork Flutter app lives under `app/`. `app/` holds `.env.example`,
...
**two** paths that CoWork already has at the root: `.gitignore` and `README.md`.
Both are handled by renaming the CoWork root copies out of the way first, so the
...
| `package:cowork/` occurrences left | **0** |
| Files carrying `package:chuk_chat/` at HEAD | 412 |
| `name: cowork` in `pubspec.yaml` | rewritten to `name: chuk_chat` in every commit |
...
With the paths aligned, merge `upstream/master` and `cowork` with **no common
...
| Pure upstream drift (CoWork side equals `d31526a`) | **16** | take upstream |
| Pure CoWork divergence (upstream side equals `d31526a`) | **83** | take CoWork |
...
| Bucket | Files |
|---|---:|
| CoWork-only, pure addition | **920** (317 of them Python) |
...
add/add pairs with no ancestor at `d31526a` (upstream and CoWork each invented a
file at the same path). **Zero** conflicts come from pure upstream drift — drift
alone always merges clean, because the CoWork side still equals the baseline.
...
| hunks | upstream +/- vs `d31526a` | CoWork +/- vs `d31526a` | file |
|---:|---:|---:|---|
...
| hunks | kind | upstream +/- vs base | CoWork +/- vs base | file |
|---:|---|---:|---:|---|
...
All nine files exist upstream and in CoWork at their correct paths, and all nine
...
exists at `d31526a` and in CoWork, and upstream deleted it in `52eb2840`
...
What that means for the merge: an unrelated-histories merge **keeps** the CoWork
file, because nothing deletes it. So the merged tree holds both
`desktop_chat_widgets.dart` (dead, CoWork's, imported by nobody after the
...
| Directory | CoWork files | chuk_chat master files | Collisions |
|---|---:|---:|---:|
| `agent/` | 100 | 0 | 0 |
| `executor/` | 59 | 0 | 0 |
| `host/` | 57 | 0 | 0 |
...
`scripts/` (7 CoWork files, 12 upstream files) shares the directory but no
filename.
...
| Path | chuk_chat master | CoWork | Take |
|---|---|---|---|
| `.codex` | empty regular file | directory (`config.toml`, `hooks.json`) | CoWork's directory |
...
Six identity-bearing files fall in the "pure CoWork divergence → take CoWork"
bucket. Taking CoWork's version there would rename and re-icon the **shipped**
...
|---|---|---|
| `linux/CMakeLists.txt` | take CoWork | `BINARY_NAME "cowork"`, `APPLICATION_ID dev.chuk.cowork` |
| `linux/runner/my_application.cc` | take CoWork | window title |
...
| `android/app/src/main/res/mipmap-*/ic_launcher.png` (5) | take CoWork | launcher icon |
| `.metadata` | take CoWork | Flutter project id |

There is also exactly one stale duplicate:
`android/app/src/main/kotlin/dev/chuk/cowork/MainActivity.kt` is CoWork-only and
sits next to upstream's `android/app/src/main/kotlin/dev/chuk/chat/MainActivity.kt`.
Delete the CoWork one; upstream's package is the one the manifest names.
...
git clone --no-hardlinks --single-branch -b cowork \
...
package:cowork/==>package:chuk_chat/
regex:(?m)^name: cowork$==>name: chuk_chat
EOF

~/.local/bin/git-filter-repo --force \
  --path-rename .gitignore:.gitignore.cowork-root \
  --path-rename README.md:README.cowork.md \
  --path-rename app/: \
  --replace-text /tmp/cowork-replace.txt

# gates
test "$(git rev-list --count HEAD)" = 306
test -z "$(git grep -l 'package:cowork' HEAD -- || true)"
...
git merge --allow-unrelated-histories --no-commit cowork
```
...
for p in $(git diff --name-only --diff-filter=U); do
  ...
  cw=$(git rev-parse --verify -q "cowork:$p")            || cw=
  ...
  if   [ -n "$ba" ] && [ "$cw" = "$ba" ]; then git checkout --ours   -- "$p"
    ...
    git merge-file -L 'upstream(master)' -L 'baseline(d31526a)' -L 'cowork' \
...
In this merge `--ours` is **upstream/master** and `--theirs` is **cowork**. The
`rm -f` before the copy is not decoration: CoWork's `AGENTS.md` is a symlink to
`CLAUDE.md`, and a plain `cp` writes through it and destroys `CLAUDE.md`.

Measured outcome: 16 take upstream, 83 take CoWork, 29 merge clean, **78 files
...
CoWork's `target-platform` ABI logic and `ic_notification.xml` / `colors.xml`
additions.
...
2. `pubspec.yaml` (6 hunks): take the **union** of dependencies. CoWork added
...
3. The four `lib/l10n/strings_*.dart` files (11, 3, 3 hunks): both sides added
   keys. Take both sets.
4. `lib/pages/*` and `lib/widgets/*` (about 30 files, 2–12 hunks): CoWork's edits
   ...
   structure and re-apply CoWork's styling on top.
5. `lib/widgets/sidebar/sidebar_chrome.dart` (19 hunks, 1044 upstream lines
   added): almost pure upstream rework. Take upstream, re-apply CoWork's 112
   lines.
6. `lib/platform_specific/chat/chat_ui_desktop.dart` (11 hunks) and
   ...
   the `chat_message_list_item.dart` split), then port CoWork's changes hunk by
...
| `scripts/capture_app_window.sh` | `$project_root/app/build/linux/…/cowork` → `$project_root/build/linux/…/chuk_chat` |
...
`.gitignore` and delete it; fold `README.cowork.md` into `README.md` and delete
...
scripts/build_apk.sh --emulator                    # CoWork mode still builds
```

Commit as a real merge, so both histories stay reachable:

```bash
git commit -m "merge(cowork): one app — CoWork rides on chuk_chat master"
...
* The CoWork commits exist on `chuk-development/chuk_chat` under **new SHAs**.
  `filter-repo` rewrote all 306. Any clone that still holds the old SHAs
  (`/home/user/git/cowork`, and the `cowork` branch already on the remote) is
  orphaned from the new history.
* **Leave the existing remote `cowork` branch alone.** Do not force-push over it.
  It is the only copy of the pre-rewrite SHAs and it costs nothing to keep. Tag
  it before the push: `git push upstream refs/remotes/origin/cowork:refs/tags/cowork-pre-merge`
