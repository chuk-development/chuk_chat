# Agents — how user credentials are handled today (audit, 2026-09-13)

Read-only audit of `/home/user/git/cowork` @ branch `agents` (HEAD `f694826`).
...
**Headline: Agents already has a complete, purpose-built "secret request" path.**
...
- Tool definition: `agent/src/chuk_agents_runtime/secrets.py:168` (`REQUEST_SECRETS_SCHEMA`),
  handler `:214` (`make_request_secrets_handler`), registration `:266`.
- Executor emits the frame and BLOCKS the worker thread:
  `executor/src/chuk_agents_executor/executor.py:2764` (`_request_secrets`), timeout
  `SECRET_REQUEST_TIMEOUT = 600.0` at `:353`.
- Frame builder: `executor/src/chuk_agents_executor/protocol.py:758`
  (`secret_request_payload`).
- App receives it: `app/lib/services/agents/agents_relay_client.dart:2599`
  (`case 'secret_request'`), model class `AgentsRelaySecretRequest` at `:1075`.
- UI: `app/lib/widgets/agents_thread_view.dart:1500` (`_buildSecretRequestBar`),
...
- OAuth connectors: tokens minted in-app, forwarded to the host inside the
  ...
  (`executor/src/chuk_agents_executor/protocol.py:155` docstring, `:203`).
- Rotation flows back as `mcp_credentials`
  (`executor/src/chuk_agents_executor/protocol.py:688`; `client_secret` is
...
- The paired account bearer lives in `host/src/chuk_agents_host/account_store.py`
  (`TOKEN_FIELDS` at `:41`), a `0600` file next to `paired.json`.
- Config explicitly refuses to hold secrets:
  `common/chuk_agents_config/src/chuk_agents_config/schema.py:32` documents the
  `*_key_ref` / `*_token_ref` convention (e.g. `model.api_key_ref` at `:454`,
  `memory.embed_api_key_ref` at `:529`), and
  `common/chuk_agents_config/tests/test_no_secrets.py:30` fails the build if a new
...
1. **Tool results never carry a value.** `request_secrets` and `list_secrets`
   ...
   (`agent/src/chuk_agents_runtime/secrets.py:205` `status_map`, and the defensive
...
2. **The `Scrubber` masks values in everything flowing back.**
   `agent/src/chuk_agents_runtime/secrets.py:97` (`class Scrubber`). It masks the raw
   ...
   - registry dispatch result — `agent/src/chuk_agents_runtime/secrets.py:275`
     (`registry.result_filter = Scrubber(access.env).scrub_obj`), applied in
     `agent/src/chuk_agents_runtime/registry.py:226`–`:235`.
   - the message-row write path (so the stored transcript is masked too) —
     `agent/src/chuk_agents_runtime/runtime.py:619` (`persist_filter=...`), applied in
     `agent/src/chuk_agents_runtime/loop.py:312`–`:316`.
   - the executor's frame sealer, so the **app** also only ever sees masks —
     `executor/src/chuk_agents_executor/executor.py:1661`–`:1669`, scrubber built at
     `:793`.
3. **Values never reach a child's shell state.** They are passed only as the
   ...
   `agent/src/chuk_agents_runtime/tools.py:187` (`_secret_env`), `:223`, `:320`.
   Docker path: `sandbox/src/chuk_agents_sandbox/docker.py:444` uses `docker exec -e
   NAME` **with no value**, so the value rides the docker client's own env and
   never appears in argv/`ps`/history. Local path:
   `sandbox/src/chuk_agents_sandbox/local.py:79`. The base env's contract is spelled
   out in `agent/src/chuk_agents_runtime/environment.py:57`–`:63` and
   `sandbox/src/chuk_agents_sandbox/base.py:80`, `:202` (the `declare -px` session
   snapshot is taken **after** the names are unset).
4. **The prompt tells the model this is deliberate**:
   `agent/src/chuk_agents_runtime/prompt.py:144` ("Never print secrets…") and the
   builtin skill `skills/builtin/secrets/SKILL.md`.

**Heuristic second net (different path, do not confuse it with the scrubber):**
`agent/src/chuk_agents_runtime/context.py:227` `redact_secrets()` — regex patterns for
...
`executor/src/chuk_agents_executor/executor.py:3223`. It is *not* on the ordinary
tool-result path.
...
`executor/src/chuk_agents_executor/protocol.py:15`–`:130` and the builders are the
`*_payload` functions in that file. The Dart switch that consumes them is
`app/lib/services/agents/agents_relay_client.dart:2516`–`:2694`.

| kind | Python builder (`executor/…/protocol.py`) | Dart model (`…/agents_relay_client.dart`) |
|---|---|---|
| `delta` | `:357` | `AgentsRelayDelta` `:168` |
| `user` | (replay) | `AgentsRelayUser` `:194` |
| `reasoning` | `:361` | `AgentsRelayReasoning` `:213` |
| `tool` | `:372` | `AgentsRelayTool` `:249` |
| `file` | `:383` (`MAX_FILE_BYTES` `:150`) | `AgentsRelayFile` `:393` |
| `subagent` | `:417` | `AgentsRelaySubagent` `:590` |
| `approval_request` / `approval_decision` | `:283` / `:320` | `AgentsRelayApprovalRequest` `:970` |
| **`secret_request`** | **`:758`** | **`AgentsRelaySecretRequest` `:1075`** |
| **`secrets`** (app→host) | **`:733`** | `sendSecrets` **`:2307`** |
| `run_state` / `run_ack` | `:224` / `:256` | `AgentsRelayRunState` `:526` |
| `done` / `error` | `:522` / `:726` | `AgentsRelayDone` `:423` / `AgentsRelayRunError` `:903` |
| `automation` / `automation_list` / `automation_control` | `:567`, `:573`, `:578` | `:637`, `:688` |
| `agent_create/rename/list` | `:586`, `:592`, `:602` | `AgentsRelayAgentList` `:825` |
| `agent_status` | `:636` | `AgentsRelayAgentStatus` `:757` |
| `skills_list` / `skill_control` | `:610` / `:620` | `AgentsRelaySkillsList` `:718` |
| `mcp_tools` / `mcp_credentials` | `:663` / `:688` | dispatch `:2660` / `:2664` |
| `documents` / `documents_list` / `document_read` | — | `AgentsRelayDocuments` `:1317` |
...
| `debug_context` | `:774` | `AgentsRelayDebugContext` `:914` |
...
(`app/lib/widgets/agents_thread_view.dart:1276`–`:1281`), they are **not**
...
(`executor/src/chuk_agents_executor/executor.py:1737` →
`_flush_pending_secret_requests` at `:2722`). The run ledger records an approval
as a synthetic completed `ask_user` tool call
(`app/lib/services/agents/agents_run_ledger.dart:684`); **no equivalent exists
...
- The host holds one `SecretsVault` per user, built in
  `host/src/chuk_agents_host/host.py:238`–`:242`, loaded from disk at `:242`.
- Values are injected per command only:
  `agent/src/chuk_agents_runtime/tools.py:187` (`_secret_env`) → `env.run_bash(..., env=…)`
  → `sandbox/src/chuk_agents_sandbox/docker.py:444` (`docker exec -e NAME`, value from
  the client's own env, never argv) or `sandbox/src/chuk_agents_sandbox/local.py:79`.
- Background/tmux jobs get the same seam:
  `agent/src/chuk_agents_runtime/shell_tools.py:249`, `:276`–`:279`.
- Subagents inherit the parent's seam:
  `executor/src/chuk_agents_executor/executor.py:3738`–`:3743`.
- Room turns get it too: `host/src/chuk_agents_host/host.py:258`.
...
- MCP servers are the exception: their bearer is attached as an
  ...
  (`agent/src/chuk_agents_runtime/mcp_client.py:739`–`:762`), never exported to the shell.
...
- `app/lib/services/secrets/secrets_store.dart:60` — the whole set is one record,
  key `agents_secrets_v1`, in `flutter_secure_storage` via
  `FlutterSecureKeyValueStore` (defined in
  `app/lib/services/agents/agents_pairing_store.dart`).
...
**Host (Python)**
- `~/.agents/secrets.enc`, AES-256-GCM, `0600`, written atomically:
  `executor/src/chuk_agents_executor/secrets.py:171` (`save`), `:206` (`load`),
  AAD `b"cowork/host/secrets-at-rest/v1"` at `:246`.
- The key is **derived**, not stored: HKDF-SHA256 over the host's Ed25519 device
  seed — `host/src/chuk_agents_host/secrets_key.py:26` (`secrets_at_rest_key`).
  Deleting `host_device.key` makes the vault unreadable, which is the intended
  "this host is no longer mine" behaviour.
- `forget_at_rest()` at `executor/src/chuk_agents_executor/secrets.py:237`.
...
1. `app/lib/widgets/agents_thread_view.dart` — move `_buildSecretRequestBar`
   (`:1500`) out of the pinned area (`:1276`–`:1281`) and render it as a list
   item in the transcript; drop the `topInset` bump at `:1434`.
2. `app/lib/services/agents/agents_run_ledger.dart` — add a secret-request entry
...
3. `app/lib/services/agents/agents_relay_client.dart` — `AgentsRelaySecretRequest`
   (`:1075`) needs whatever extra fields the inline card wants (a per-name label
   or hint); `sendSecrets` at `:2307` stays as is.
4. `executor/src/chuk_agents_executor/protocol.py:758` (`secret_request_payload`) and
   `:733` (`secrets_payload`) — only if new optional fields are added.
5. `executor/src/chuk_agents_executor/executor.py` — `_request_secrets` (`:2764`) and
   `_flush_pending_secret_requests` (`:2722`) if the card must survive a replay
   as a *stored* row rather than an in-memory re-send.
6. `agent/src/chuk_agents_runtime/secrets.py:168` (`REQUEST_SECRETS_SCHEMA`) — only if
...
8. Tests that pin the current behaviour:
   `app/test/widgets/agents_thread_view_secrets_test.dart`,
   `app/test/services/agents/agents_relay_secrets_test.dart`,
...
(`agent/src/chuk_agents_runtime/secrets.py:97`), `result_filter`
