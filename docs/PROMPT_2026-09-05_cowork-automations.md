...
3. docs/HANDOVER_2026-09-05_PYTHON.md, docs/HANDOVER_2026-09-05_REPLAY_EVENTS_266.md,
   docs/HANDOVER_2026-09-05_REASONING_TOOLFRAMES.md, docs/PLAN_2026-09-04_AGENTS_CHUK_ALIGN.md §WS-7
   (Notifications: host/notify.py, desktop_notify.py — ein Trigger-Run muss den User benachrichtigen).
4. Python: executor/src/chuk_agents_executor/executor.py (_accept_task, Worker-Queue pro Session, _run_task, Frames),
   agent/src/chuk_agents_runtime/{tools.py,registry.py,state.py (runs-Tabelle)}, host/src/chuk_agents_host/{host.py,serve.py}.
   Dart: agents_relay_client.dart, agents_thread_view.dart, agents_replay_loader.dart, settings hubs.
...
- Tools fuer das Modell: `schedule_task(spec, prompt, name?)` (spec: cron-String 5 Felder ODER `every: 300`
  ...
  supervised im Sandbox als Kindprozess; stdout/stderr in Log-Datei im Workspace `.agents/automations/<id>.log`;
...
- Self-Wake-Hook fuer Scripts: kleines Modul, das das Modell im Script importieren kann
  (`from agents_hooks import trigger; trigger("Preis unter 100", payload={...})`). Implementierung ohne Netz:
  das Script schreibt eine JSON-Zeile in eine Named-Pipe/Datei `.agents/automations/triggers.jsonl` (append,
...
- Relay-Client: case `automation` + `sendAutomationControl()`. Thread-View: Karte fuer automation-Events
  ...
  in der Agents-Sektion des Settings-Hubs (Liste aller aktiven mit next_fire/last_fired/fire_count, Pause/Cancel).
...
- Deine Dateien: neu host/src/chuk_agents_host/automations.py, agent/.../agents_hooks.py (+ Sandbox-Installationspfad),
