...
Decided 2026-09-13. "CoWork" was the working title while the platform lived in
...
| Dart package prefix | `package:cowork` | `package:chuk_chat` (the app absorbs it) |
| Python namespace | `cowork_agent`, `cowork_executor`, `cowork_host`, `cowork_manager`, `cowork_sandbox`, `cowork_crypto`, `cowork_config` | `chuk_agents.runtime`, `.executor`, `.host`, `.manager`, `.sandbox`, `.crypto`, `.config` |
| Environment prefix | `COWORK_*` | `AGENTS_*` |
| State directory | `~/.cowork` | `~/.agents` |
| Config file | `~/.cowork/config.toml` | `~/.agents/config.toml` |
| Images | `cowork-base`, `cowork-browser` | `ghcr.io/chuk-development/agents-base`, `…/agents-browser` |
| Service unit | `cowork-manager.service` | `agents-manager.service` |
| Container prefix | `cowork-<agent id>` | `agents-<agent id>` |
...
Beads issue ids stay `cowork-*`. They are identifiers, not names: they are
...
- **Environment variables**: `cowork_config` already carries each field's
  environment name as data, so the rename is one string per field. Read
  `AGENTS_*` first and fall back to `COWORK_*` with a deprecation note for one
  release, because the systemd unit, the compose files and any shell a user
  wrote still say `COWORK_*`.
- **State directory**: on start, if `~/.agents` is absent and `~/.cowork` exists,
