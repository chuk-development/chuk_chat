...
This document supersedes the *shape* of `docs/AGENTS_EXECUTION_PLAN.md` (a
single-session desktop↔phone mirror). What survives from that plan: the relay,
the `agents_frame*` crypto primitives, and the client-side approval model.
...
2. **Agent chat (Agents)** — a **messenger, like Slack**, whose contacts are
...
- **Frame contract & transport patterns** (Hermes `gateway/relay/` +
  ...
  **reconnect supervisor** one layer above a dumb read-loop (backoff, re-dial,
  ...
  specifics.
- **E2E** on the control/mirror channel via a **fresh Agents channel key**
  ...
  and putting a password on a headless host, and decouples Agents crypto from
  chat crypto. The Dart `agents_frame*` primitives get repointed from the account
...
- **Stack:** Flutter **`flutter_webrtc`** ↔ Python **`aiortc`**, **DataChannels
  only** (SCTP/DTLS/ICE) — no media tracks. Our sealed `agents_frame`s ride as
  DataChannel messages.

**What stays vs what changes:**
- **Only the PIPE changes.** The `agents_frame*` E2E crypto, the §15 pairing, and
...
in both `chuk_agents_crypto` (Python) and Dart `cryptography`). A PAKE (SPAKE2) was
...
6. On success: `K` becomes the Agents channel key (§14). Each side sends its
...
**Reconnect** (`chuk_agents_crypto.reconnect` / `agents_reconnect.dart`) replaces the
...
- **Copy only the basic security stack** from chuk_chat (~28 files; the port
  ...
  the multiplex WebSocket transport, and the `agents/` crypto primitives. Cut the
...
- **UI = a ChukChat clone.** The chat UI looks exactly like ChukChat. The one
  ...
  shows conversations, agents shows the bot roster (§16.1). **No thread list**
...
lib/services/agents/agents_frame.dart · agents_replay_guard.dart ·
  agents_device_keys.dart · agents_approved_devices.dart · agents_frame_codec.dart
```
...
`agents/agents_executor_bridge.dart` + `agents_demo_server*.dart` (the discarded
demo).
