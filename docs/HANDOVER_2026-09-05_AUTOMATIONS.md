...
commit release is the user's call. Branch `cowork`, one shared tree.
...
| File | What |
|---|---|
| `lib/services/automations/cowork_automation.dart` (new) | `CoworkAutomation` (the row as the app sees it, `fromPayload`, `specLabel`), `CoworkAutomationControl` (the two outbound frames as their own interface, so the shared test doubles keep compiling). |
| `lib/services/automations/automations_source.dart` (new) | `AutomationsSource.instance`: folds `automation` events and `automation_list` replies from the link into one map, `forSession` / `liveForSession` / `all`, `refresh()` / `control()` through the bound controller when it `is CoworkAutomationControl`. |
...
| `lib/services/cowork/cowork_relay_client.dart` | `CoworkRelayAutomation`, `CoworkRelayAutomationList` (sealed subclasses), cases `automation` / `automation_list` in `_dispatch`, `CoworkRelayClient implements CoworkAutomationControl` (`sendAutomationControl`, `requestAutomationList`). |
| `lib/services/cowork/cowork_run_ledger.dart` | `_automationCalls` + `automation(sessionKey, event)`. |
| `lib/services/cowork/cowork_replay_loader.dart` | Case `CoworkRelayAutomation` (one card per id on the answer row), `CoworkRelayAutomationList` (ignored). |
| `lib/services/websocket_chat_service.dart` | Live case → `ledger.automation`; `_isReplay` line. |
| `lib/widgets/cowork_thread_view.dart` | Two `break` cases; the automations strip above the chat (collapsible, compact cards with Pause / Resume / Cancel through the source). |
| `lib/pages/settings_page.dart`, `lib/pages/desktop_settings_modal.dart` | "Automations" after "API Keys" in the CoWork section. |

Tests (one file at a time, all green): `test/services/automations/cowork_automation_test.dart` 13,
`automations_source_test.dart` 5, `test/widgets/automation_card_test.dart` 3,
`test/pages/automations_page_test.dart` 5; regression `cowork_replay_loader_test.dart` 25.
...
- `automations` row: active, `fire_count 1`; log
  `.cowork/automations/<id>.log` ("start watch_channel.py") and
