...
The byte-identical Python twin of the Dart Agents frame crypto in chuk_chat
(`lib/services/agents/agents_frame*.dart`). It seals and opens the frames that
...
The symmetric key is **not** the chat account key. It is a fresh Agents
...
from chuk_agents_crypto import (
    ...
    AgentsFrameSealer, AgentsFrameOpener, seal, open,
...
# seal / open
sealer = AgentsFrameSealer(channel_key=k, key_version=1,
                           device_id="executor-01", signing_identity=ident)
frame_bytes = sealer.seal(b"payload")      # -> wire bytes
opener = AgentsFrameOpener(channel_key=k, key_version=1, approved_devices=approved)
plaintext = opener.open(frame_bytes)       # -> b"payload", or raises AgentsFrameRejected
...
`open` raises `AgentsFrameRejected` with a `.rejection` of type
`AgentsFrameRejection` on any failure: `MALFORMED`, `UNSUPPORTED_VERSION`,
...
2. build a `AgentsFrameSealer` from `ed25519_seed_b64` and `channel_key_b64`,
...
Python side only — treat it as unconfirmed on the wire. The Dart `agents_frame*`
