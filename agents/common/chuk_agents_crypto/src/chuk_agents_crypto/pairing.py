...
Python twin of ``agents_pairing.dart``. Implements §15 of the platform plan:
...
Channel key (the key that later seals frames, §14)
    ...
    — reuses :func:`chuk_agents_crypto.channel_key.derive_channel_key` verbatim, so
...
# --- byte-exact protocol constants (mirror agents_pairing.dart) --------------
...
class Pairing:
    ...
    def channel_key(self) -> bytes:
        """The 32-byte Agents channel key — **only** after both confirmation
