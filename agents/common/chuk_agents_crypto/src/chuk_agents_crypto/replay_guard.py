"""Replay protection for one peer device — Python twin of ``AgentsReplayGuard``.
...
Mirrors ``agents_replay_guard.dart``.
"""
...
from .frame import AgentsFrame, AgentsFrameRejected, AgentsFrameRejection
...
class ReplayGuard:
    ...
    def check(self, frame: AgentsFrame) -> None:
    ...
    def commit(self, frame: AgentsFrame) -> None:
    ...
    def _check_timestamp(self, frame: AgentsFrame) -> None:
        skew = self._now_ms() - frame.ts
        if abs(skew) > self.window_ms:
            raise AgentsFrameRejected(AgentsFrameRejection.TIMESTAMP_OUT_OF_WINDOW)

    def _check_seq(self, frame: AgentsFrame) -> None:
        last = self._last_seq
        if last is not None and frame.seq <= last:
            raise AgentsFrameRejected(AgentsFrameRejection.REPLAYED_SEQUENCE)
