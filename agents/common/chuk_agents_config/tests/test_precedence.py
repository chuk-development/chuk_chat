...
test suites all export ``AGENTS_*`` variables today. If the file could beat
...
from chuk_agents_config import ConfigError, get_value, load_config
...
def test_environment_beats_file(tmp_path):
    path = write(tmp_path, "version = 1\n[model]\ndefault = \"from-file\"\n")
    config = load_config(path=path, environ={"AGENTS_MODEL_DEFAULT": "from-env"})
...
def test_argument_beats_environment(tmp_path):
    ...
    config = load_config(
        path=path,
        environ={"AGENTS_MODEL_DEFAULT": "from-env"},
...
def test_all_four_layers_at_once(tmp_path):
    ...
    config = load_config(
        path=path,
        environ={"AGENTS_MODEL_DEFAULT": "from-env", "AGENTS_VNC_PORT": "5902"},
        overrides={"model.default": "from-argument"},
    )
...
@pytest.mark.parametrize(
    ("env", "path", "expected"),
    [
        ({"AGENTS_SANDBOX_IMAGE": "custom:9"}, "sandbox.image", "custom:9"),
        ({"AGENTS_SANDBOX_KIND": "docker"}, "sandbox.kind", "docker"),
        ({"AGENTS_DESKTOP_NOTIFY": "0"}, "notify.desktop", False),
        ({"AGENTS_RUN_MAX_SECONDS": "7200"}, "limits.run_max_seconds", 7200.0),
        ({"AGENTS_VNC_XDAMAGE": "0"}, "vnc.xdamage", False),
        ({"AGENTS_BROWSER_AUTO_OPEN": "false"}, "browser.auto_open", False),
        ({"AGENTS_BROWSER_TARGET": "user_browser"}, "browser.target", "user_browser"),
    ],
)
...
def test_empty_environment_variable_counts_as_unset(tmp_path):
    ...
    config = load_config(path=tmp_path / "config.toml", environ={"AGENTS_BROWSER_DISPLAY": ""})
    assert config.browser.display == ":99"


def test_environment_boolean_words(tmp_path):
    for word in ("1", "true", "TRUE", "yes", "on"):
        config = load_config(path=tmp_path / "c.toml", environ={"AGENTS_BROWSER_HEADLESS": word})
...
def test_bad_environment_value_is_named_not_ignored(tmp_path):
    with pytest.raises(ConfigError) as caught:
        load_config(path=tmp_path / "c.toml", environ={"AGENTS_VNC_PORT": "five thousand"})
    assert caught.value.paths() == ("vnc.port",)
    assert "AGENTS_VNC_PORT" in str(caught.value)
