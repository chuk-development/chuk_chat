"""The schema must cover every ``AGENTS_*`` variable in the repository.

This test greps the tree the same way the inventory did:

    grep -rhoE "AGENTS_[A-Z0-9_]+" --include="*.py" --include="*.sh" \\
        agents/runtime/src agents/executor/src agents/manager/src \\
        agents/sandbox agents/host scripts | sort -u

Every name it finds must be either a field of the schema or an entry in
:data:`chuk_agents_config.NOT_CONFIG` with a written reason. That is the point of
the test: the schema cannot quietly fall behind the code. A new
``AGENTS_SOMETHING`` added anywhere in the tree fails this test until somebody
...
from chuk_agents_config import FIELDS_BY_ENV, NOT_CONFIG
...
NAME_RE = re.compile(r"AGENTS_[A-Z0-9_]+")


def repo_root() -> Path | None:
    # tests/ -> chuk_agents_config/ -> common/ -> agents/ -> the repository
    root = Path(__file__).resolve().parents[4]
    return root if (root / "agents" / "runtime" / "src").is_dir() else None
...
def test_every_environment_variable_is_decided():
    ...
    assert undecided == [], (
        "these AGENTS_ variables are in the tree but not in the schema: "
        + ", ".join(undecided)
...
def test_every_exclusion_has_a_reason():
    for name, reason in NOT_CONFIG.items():
        assert name.startswith("AGENTS_")
