...
def _every_field_changed() -> AgentsConfig:
...
def test_saved_file_is_private(tmp_path):
    path = save_config(AgentsConfig(), tmp_path / "config.toml")
...
def test_saved_file_replaces_an_existing_one_and_stays_private(tmp_path):
    ...
    target.chmod(0o644)
    save_config(set_value(AgentsConfig(), "vnc.port", 5901), target)
...
def test_save_creates_the_state_directory(tmp_path):
    path = save_config(AgentsConfig(), tmp_path / "deep" / "state" / "config.toml")
    assert path.exists()


def test_file_starts_with_the_version(tmp_path):
    text = to_toml(AgentsConfig())
...
def test_only_changed_values_can_be_written(tmp_path):
    config = set_value(AgentsConfig(), "model.default", "claude-opus-5")
...
def test_set_value_refuses_a_bad_value():
    with pytest.raises(ConfigError) as caught:
        set_value(AgentsConfig(), "sandbox.kind", "podman")
    assert caught.value.paths() == ("sandbox.kind",)


def test_unknown_paths_are_refused_by_both_accessors():
    with pytest.raises(ConfigError):
        get_value(AgentsConfig(), "model.defualt")
    with pytest.raises(ConfigError):
        set_value(AgentsConfig(), "model.defualt", "x")


def test_export_environ_carries_only_what_was_chosen():
    config = set_value(AgentsConfig(), "vnc.port", 5901)
    exported = export_environ(config)
    assert exported == {"AGENTS_VNC_PORT": "5901"}
...
def test_export_environ_formats_the_way_the_shell_reads_it():
    ...
    exported = export_environ(config)
    assert exported["AGENTS_VNC_XDAMAGE"] == "0"
    assert exported["AGENTS_BROWSER_HEADLESS"] == "1"
    assert exported["AGENTS_RUN_MAX_SECONDS"] == "30.0"
