"""Every Agents setting, in one place, with its meaning next to it.

Agents grew without a configuration file. Each process read what it needed
...
``AGENTS_*`` variables spread over ``host/``, ``executor/``, ``manager/``,
...
:class:`AgentsConfig`. The dataclasses are frozen because a configuration that
...
#: Agents can no longer be read field-for-field: a key was renamed, a type
...
#: :mod:`chuk_agents_config.loader`: a function from the raw table of version ``n``
...
#: refused with a named error rather than half-read: a newer Agents wrote it,
...
class SandboxConfig:
    ...
    image_digest: str = setting(
        "",
        env="AGENTS_SANDBOX_IMAGE_DIGEST",
        doc=(
    ...
    uid: int = setting(
        ...
        doc=(
            ...
            "started Agents and not to root. ``0`` means take the id of the "
...
class BrowserConfig:
    ...
    profile: str = setting(
        "/workspace/.agents/chrome-profile",
        env="AGENTS_BROWSER_PROFILE",
        doc=(
    ...
    cursor_theme: str = setting(
        "DMZ-White",
        env="AGENTS_BROWSER_CURSOR_THEME",
        doc=(
    ...
    cursor_size: int = setting(
        64,
        env="AGENTS_BROWSER_CURSOR_SIZE",
        doc=(
    ...
    retired_hostname: str = setting(
        "",
        env="AGENTS_BROWSER_RETIRED_HOSTNAME",
        doc=(
    ...
    extension_mcp: str = setting(
        "",
        env="AGENTS_EXTENSION_MCP",
        doc=(
            "Path to the ``agents-extension-mcp`` server that drives the "
...
class ModelConfig:
    ...
    reasoning_effort: str = setting(
        "",
        env="AGENTS_MODEL_REASONING_EFFORT",
        doc=(
...
class MemoryConfig:
    ...
    llm_model: str = setting(
        "agents-memory-writer",
        env="AGENTS_MEM_LLM_MODEL",
        doc=(
    ...
    embed_api_key_ref: str = setting(
        "",
        env="AGENTS_MEM_EMBED_API_KEY_REF",
        doc=(
...
class NotifyConfig:
    ...
    desktop: bool = setting(
        True,
        env="AGENTS_DESKTOP_NOTIFY",
        doc=(
            "Show a desktop notification on the machine Agents runs on. Off "
...
class LimitsConfig:
    ...
    run_ack_timeout_seconds: float = setting(
        15.0,
        env="AGENTS_RUN_ACK_TIMEOUT_SECONDS",
        doc=(
    ...
    approval_wait_seconds: float = setting(
        600.0,
        env="AGENTS_APPROVAL_WAIT_SECONDS",
        doc=(
...
class AutomationConfig:
    ...
    triggers_path: str = setting(
        ".agents/automations/triggers.jsonl",
        env="AGENTS_TRIGGERS_PATH",
        doc=(
...
class EmulatorConfig:
    ...
    through :func:`chuk_agents_config.loader.export_environ`.
    """
...
class AgentsConfig:
    ...
    Build it with :func:`chuk_agents_config.load_config`; it is frozen, so change it
    with :func:`chuk_agents_config.set_value`, which returns a new one.
    """
...
def _build_sections() -> dict[str, type]:
    found: dict[str, type] = {}
    for entry in fields(AgentsConfig):
...
#: ``"AGENTS_..." -> spec``. Two settings may never share a variable.
...
#: ``AGENTS_*`` names that appear in the source tree and are deliberately **not**
...
NOT_CONFIG: dict[str, str] = {
    ...
    "AGENTS_AGENT_PLATFORM_PLAN": (
        "Not a variable. It is part of the filename "
        "``docs/AGENTS_AGENT_PLATFORM_PLAN.md`` in a docstring."
    ),
    "AGENTS_CWD": (
        "Not a variable. It is part of the random shell marker "
        "``__AGENTS_CWD_<hex>__:`` the local sandbox prints to find the "
        "working directory after a command."
    ),
    "AGENTS_CWD_": "Not a variable. The same shell marker, matched one character longer.",
    "AGENTS_RELAY": (
        "Not a variable. It is the Python constant ``TYPE_AGENTS_RELAY``, the "
        "name of a frame type on the wire."
    ),
    "AGENTS_ERROR": (
        "Not a variable. It is the Python constant ``TYPE_AGENTS_ERROR``, the "
        "name of a frame type on the wire."
    ),
    "AGENTS_VNC_REVISION": (
