...
from cowork_config import ConfigError, config_home, default_config_path, load_config
...
def test_default_path_follows_cowork_home(tmp_path):
    env = {"COWORK_HOME": str(tmp_path / "state")}
...
def test_default_path_expands_a_tilde():
    assert not str(config_home({"COWORK_HOME": "~/elsewhere"})).startswith("~")
...
def test_a_file_from_a_newer_cowork_is_refused(tmp_path):
    path = tmp_path / "config.toml"
    path.write_text("version = 99\n", encoding="utf-8")
    with pytest.raises(ConfigError) as caught:
        load_config(path=path, environ={})
    assert caught.value.paths() == ("version",)
    assert "newer CoWork" in caught.value.problems[0].message
