...
``image or os.environ.get("COWORK_SANDBOX_IMAGE") or BASE_IMAGE``, repeated
...
```python
image: str = setting("", env="COWORK_SANDBOX_IMAGE", doc="The container image ...")
```
...
already do (``${COWORK_VNC_PORT:-5900}``), and it is the only rule that keeps
...
def format_env_value(spec: FieldSpec, value: Any) -> str:
    ...
    through :func:`cowork_config.loader.export_environ`, which formats every
    value here.
    """
    if spec.type is bool:
