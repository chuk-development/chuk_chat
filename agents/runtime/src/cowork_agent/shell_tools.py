...
- **An interactive shell.** ``shell_start`` / ``shell_read`` / ``shell_send`` /
  ...
  :mod:`cowork_agent.terminal`: session names, key tokens, output caps. The
...
- **Background jobs.** ``run_command(background=true)`` starts a command
  ...
  job's files under ``<workspace>/.cowork/jobs/`` and a wrapper appends ONE
  line to the automations' trigger file when the job ends. The host tails
  that file and wakes the agent (``cowork_executor.shell``). The model never
...
TRIGGERS_RELPATH = ".cowork/automations/triggers.jsonl"
...
ENV_WORKSPACE = "COWORK_WORKSPACE"
...
class JobManager:
    ...
    image's ``COWORK_WORKSPACE`` wins; locally the two are the same directory.
    """
