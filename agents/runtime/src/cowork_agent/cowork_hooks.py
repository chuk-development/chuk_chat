"""cowork_hooks — the self-wake hook a watcher script imports.

The host copies this ONE file into ``<workspace>/.cowork/automations/`` and
...
ENV_AUTOMATION_ID = "COWORK_AUTOMATION_ID"
#: Where to append. Relative paths are resolved against the current directory,
#: which the host sets to the workspace.
ENV_TRIGGERS_PATH = "COWORK_TRIGGERS_PATH"
DEFAULT_TRIGGERS_PATH = ".cowork/automations/triggers.jsonl"
...
def trigger(reason: str, payload=None, *, kind: str = "automation") -> bool:
    ...
    if aid is None:
        sys.stderr.write(
            "cowork_hooks.trigger: not running as a watcher "
    ...
    try:
        ...
        sys.stderr.write(f"cowork_hooks.trigger: cannot write {path}: {exc}\n")
        return False
    return True
