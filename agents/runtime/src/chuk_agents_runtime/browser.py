...
* **Where it looks for that binary** (``browser/watchdogs/local_browser_watchdog.py::
  ...
  — the reason the image is a separate variant (``agents/sandbox/docker/Dockerfile.browser``).
...
rather than quietly going headless. ``AGENTS_BROWSER_HEADLESS=1`` is the only
...
(``agents/sandbox/docker/browser-mcp.sh``, headed on Xvfb ``:99``), not this module —
...
:func:`chuk_agents_runtime.mcp_client.open_browser_gui` because that server launches no
...
* ``AGENTS_BROWSER_EXECUTABLE`` / a Chromium on ``PATH`` — browser-use launches
  it itself, next to the runtime.
* ``AGENTS_BROWSER_CDP_URL`` — attach to a browser somebody else runs: the
  container built from ``agents/sandbox/docker/Dockerfile.browser`` with its debug port
...
:class:`BackendChatModel` wraps any :class:`~chuk_agents_runtime.model.ModelClient` —
in production the account's :class:`~chuk_agents_runtime.backend.BackendModelClient`
...
* the start URL runs through :func:`chuk_agents_runtime.web_fetch.validate_url` — the
...
:mod:`chuk_agents_runtime.web_fetch` already says for its own residual risk. This tool
...
container built from ``agents/sandbox/docker/Dockerfile.browser`` (Google Chrome for
...
CDP_ENV_VAR = "AGENTS_BROWSER_CDP_URL"
#: Point at a specific Chromium binary instead of searching.
EXECUTABLE_ENV_VAR = "AGENTS_BROWSER_EXECUTABLE"
...
HEADLESS_ENV_VAR = "AGENTS_BROWSER_HEADLESS"
...
def find_chromium(executable: str | None = None) -> str | None:
    ...
    Order: the explicit argument, ``AGENTS_BROWSER_EXECUTABLE``, ``PATH``, then
...
class BackendChatModel:
    ...
    subclass) implemented over one of our :class:`~chuk_agents_runtime.model.ModelClient`
    instances.
...
class BrowserUseRunner:
    ...
    def __init__(
        ...
        *,
        ...
        #: says headful unless ``AGENTS_BROWSER_HEADLESS`` says otherwise. A
...
def _send_screenshots(
    ...
    *,
    ...
    ``send_file_to_user`` uses (:mod:`chuk_agents_runtime.files_out`).
...
def register_browser_task(
    ...
    *,
    ...
    the prompt (:func:`chuk_agents_runtime.prompt.render_tool_docs`) and a call to it
