...
:class:`~chuk_agents_manager.supervisor.AgentSupervisor` that does it, on top of
``chuk_agents_sandbox``'s label-keyed container lifecycle.
...
* **One environment per agent.** ``environment(agent_id)`` hands back the live
  :class:`~chuk_agents_sandbox.BaseEnvironment` for that agent, creating it on first
...
from chuk_agents_manager.supervisor import AgentSupervisor, RuntimeState, RuntimeStatus
