...
The phone-side counterpart of the :class:`~chuk_agents_executor.executor.Executor`.
...
- :func:`chuk_agents_manager.encode_frame` / :func:`chuk_agents_manager.decode_frames` for
  the envelope ser/deser,
- :func:`chuk_agents_manager.make_request` to build the ``run_task`` envelope,
- :class:`chuk_agents_manager.CorrelationMap` to match the terminal response to the
  task it opened.
"""
...
from chuk_agents_crypto import AgentsFrameOpener, AgentsFrameRejected, AgentsFrameSealer
...
class ControllerSession:
    ...
    def collect(self, request_id: str, *, timeout: float = 10.0) -> list[dict[str, Any]]:
        ...
        terminal ``done`` or ``error``). Raises :class:`AgentsFrameRejected` if a
...
__all__ = ["ControllerSession", "AgentsFrameRejected"]
