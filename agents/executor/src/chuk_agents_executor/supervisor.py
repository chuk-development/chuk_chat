...
``chuk_agents_manager`` ships the :class:`~chuk_agents_manager.AgentSupervisor` ABC and an
in-memory ``StubSupervisor``. :class:`ExecutorSupervisor` is the real thing: it
starts an :class:`~chuk_agents_executor.executor.Executor` for a roster agent as an
in-process background task and can stop it, reporting the same
:class:`~chuk_agents_manager.RuntimeState` the stub does.
...
:class:`~chuk_agents_manager.Agent` (its ``workspace_dir`` becomes the sandbox
workdir).
"""
