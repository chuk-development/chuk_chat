...
``chuk_agents_runtime`` runs shell through an ``Environment`` whose sole method is
``run_bash(cmd, *, timeout=120) -> ProcessResult``. ``chuk_agents_sandbox``'s
...
handler reads ``result.duration_s`` (``chuk_agents_runtime/tools.py:34``), but the
...
from chuk_agents_runtime import ProcessResult as AgentProcessResult
