...
The agent side (``chuk_agents_runtime.shell_tools``) starts a job detached in the
sandbox; a wrapper appends one ``kind: job`` line to the automations' trigger
file when the job ends. The host tails that file (``chuk_agents_host.automations``)
...
1. reads the job's files from the workspace on the host side (the workspace is
   a bind mount, so ``<workspace>/.agents/jobs/`` is the same directory the
...
logger = logging.getLogger("agents.executor.jobs")

#: Same layout as ``chuk_agents_runtime.shell_tools`` (kept in sync by the contract).
...
class JobWakeRouter:
    ...
    :class:`chuk_agents_executor.executor.Executor`; the router uses its
