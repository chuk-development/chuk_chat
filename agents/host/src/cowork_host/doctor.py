...
_INITIALIZE = {
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "cowork-doctor", "version": "1"},
    },
}
...
def check_image(binary: str = "docker") -> tuple[Check, str]:
    ...
    from cowork_sandbox.docker import BASE_IMAGE, IMAGE_ENV_VAR, default_image
...
def check_browser_launcher(image: str, binary: str = "docker") -> Check:
    ...
    from cowork_sandbox.docker import BROWSER_MCP_PATH, image_has_browser
...
def check_browser_speaks_mcp(image: str, binary: str = "docker") -> Check:
    ...
    from cowork_sandbox.docker import BROWSER_MCP_PATH
    ...
    try:
        ...
        return Check(
            ...
            "check the image: cowork-browser-mcp starts Xvfb and a Chromium",
        )
    except OSError as exc:
...
def cmd_doctor(args, out: Callable[..., None] = print) -> int:
    workspace = getattr(args, "workspace", None) or os.environ.get(
        "COWORK_HOME", str(Path.home() / ".cowork")
    )
