...
:class:`~chuk_agents_host.party.HostParty` drives the §15 ceremony and then serves
...
- :class:`PartyLink` carries the *party protocol* messages of
  :mod:`chuk_agents_host.protocol` as plain dicts, in both directions.

Two implementations exist:

- :class:`LocalRelayTransport` — the blind loopback relay (:mod:`chuk_agents_host.relay`).
  Same-machine development only. Its hello is the ``join`` message.
- :class:`~chuk_agents_host.cloud_relay.CloudRelayTransport` — the production pipe
