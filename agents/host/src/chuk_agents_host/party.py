...
:class:`~chuk_agents_host.transport.PartyTransport` (the blind loopback relay for
...
:class:`~chuk_agents_crypto.AgentsFrameOpener` is then handed to the Executor, which
...
TaskServerBuilder = Callable[
    [AgentsFrameOpener, AgentsFrameSealer, dict, "HostParty"], TaskServer
]
...
class HostParty:
    ...
    def start(self) -> None:
        self._thread = threading.Thread(
            target=self._run, name="agents-host-party", daemon=True
        )
        self._thread.start()
    ...
    def _on_reconnected(self, handshake: ReconnectHandshake) -> None:
        ...
        sealer = self._sealer or AgentsFrameSealer(
            channel_key=channel_key,
            key_version=self._key_version,
            device_id=self._device_id,
