...
2. **pairing** (the §15 ceremony, hand-carried through the relay)::
   ...
   ``data`` is the exact dict a :class:`chuk_agents_crypto.Pairing` step produced or
...
3. **frame** (everything after pairing: token provisioning, tasks, results)::
       ...
       {"type": "frame", "frame": "<base64 of a sealed Agents frame>"}
...
def frame_envelope(frame_b64: str) -> dict[str, Any]:
    """Wrap a base64 sealed Agents frame in its relay envelope."""
