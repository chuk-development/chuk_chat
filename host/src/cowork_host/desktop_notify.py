...
a ``cowork.desktop`` file is installed, which a host running from a checkout or
a pip install does not have. So the app logo ships with this package
(``data/cowork.png``, the app's launcher icon) and is passed by absolute path;
``COWORK_NOTIFY_ICON`` overrides it for an install that has its own themed
icon.
"""
...
PACKAGED_ICON = Path(__file__).with_name("data") / "cowork.png"


def icon_path() -> str | None:
    """The absolute path (or icon-theme name) the toast should draw, or None."""
    override = os.environ.get("COWORK_NOTIFY_ICON")
    if override:
        return override
