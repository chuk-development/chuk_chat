...
lives in ``cowork_host.automations``. The two meet at
...
#: Where the host installs ``cowork_hooks.py``, the log files and the trigger
#: file, relative to the workspace. Shared by the host and the hook module.
AUTOMATIONS_DIRNAME = ".cowork/automations"
...
def _check_interval(seconds: int) -> None:
    if seconds < MIN_INTERVAL_SECONDS:
        raise AutomationSpecError(
            ...
            "cowork_hooks.trigger() only on a change"
        )
...
START_WATCHER_SCHEMA = {
    ...
    "description": (
        ...
        "watches and calls `cowork_hooks.trigger(reason, payload=...)` ONLY "
        ...
        "stdout/stderr go to .cowork/automations/<id>.log."
    ),
    "properties": {
        "script_path": {
            "type": "string",
